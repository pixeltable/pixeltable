...
.make-install/env:
	...
	@mamba install -q -y -c conda-forge libiconv 'ffmpeg==$(FFMPEG_VERSION)' quarto 'nodejs>=20' lychee 'cmake>=3.22'
...
slimpytest: install
	...
	@$(ULIMIT_CMD) pytest $(PYTEST_COMMON_ARGS) \
	    tests/test_{alter_column,catalog,dirs,env,exprs,function,index,snapshot,table,table_model,types,view}.py
