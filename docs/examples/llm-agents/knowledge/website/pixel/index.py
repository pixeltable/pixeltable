...
def create_index(index_name: str, chunks_name: str, purge: bool = False) -> tuple[pxt.Table, pxt.Table]:
    ...
    if index_name not in pxt.list_tables():
        ...
        document_table = pxt.create_table(index_name, {'website': pxt.Document}, if_exists='ignore')

        # Create view that chunks PDFs into sections
        chunks_view = pxt.create_view(
            chunks_name,
            document_table,
            iterator=DocumentSplitter.create(document=document_table.website, separators='token_limit', limit=300),
            if_exists='ignore',
        )
        ...
        return document_table, chunks_view
    
    else:
        document_table = pxt.get_table(index_name)
