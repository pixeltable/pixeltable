...
# Get website index
website_embedding_index = pxt.get_table(f'{DIRECTORY}.website_chunks')


# Create query-as-a-tool for Agentic RAG
@pxt.query
def search_website(query_text: str) -> str:
    """
    Search tool to find relevant website passages.
    Args:
        query_text: The search query
    Returns:
        Top 10 most relevant passages
    """
    similarity = website_embedding_index.text.similarity(query_text)
    return (
        website_embedding_index.order_by(similarity, asc=False)
        .select(website_embedding_index.text, sim=similarity)
        .limit(10)
    )


agent_tools = pxt.tools(search_website)
