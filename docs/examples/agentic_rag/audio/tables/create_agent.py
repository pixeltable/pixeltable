...
# Get audio index
audio_embedding_index = pxt.get_table(f'{DIRECTORY}.audio_sentence_chunks')


# Create query-as-a-tool for Agentic RAG
@pxt.query
def search_audio(query_text: str) -> str:
    """
    Search tool to find relevant audio passages.
    Args:
        query_text: The search query
    Returns:
        Top 10 most relevant passages
    """
    similarity = audio_embedding_index.text.similarity(query_text)
    return (
        audio_embedding_index.order_by(similarity, asc=False)
        .select(audio_embedding_index.text, sim=similarity)
        .limit(10)
    )


agent_tools = pxt.tools(search_audio)
