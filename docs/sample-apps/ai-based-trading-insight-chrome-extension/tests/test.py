...
def sample_image() -> Path:
    ...
    if not image_path.exists():
        raise FileNotFoundError('Test chart image not found. Please place test_chart.png in test-data directory')
    return image_path
...
def analysis_payload(sample_image):
    ...
    return {'screenshot': f'data:image/jpeg;base64,{base64_image}', 'requestId': TestConfig.TEST_REQUEST_ID}
...
class TestTradingAnalysis:
    ...
    def test_analyze_endpoint_response_structure(self, base_url, analysis_payload):
        ...
        response = requests.post(f'{base_url}/analyze', json=analysis_payload)
    ...
    def test_analyze_endpoint_error_handling(self, base_url, bad_payload, expected_status):
        ...
        response = requests.post(f'{base_url}/analyze', json=bad_payload)
