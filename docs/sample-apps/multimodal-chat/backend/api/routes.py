...
docs_table = pxt.create_table(
    'chatbot.documents', {'document': pxt.Document, 'video': pxt.Video, 'audio': pxt.Audio, 'question': pxt.String}
)

conversations = pxt.create_table(
    'chatbot.conversations', {'role': pxt.String, 'content': pxt.String, 'timestamp': pxt.Timestamp}
)
...
docs_table.add_computed_column(transcription=openai.transcriptions(audio=docs_table.audio_extract, model='whisper-1'))
docs_table.add_computed_column(audio_transcription=openai.transcriptions(audio=docs_table.audio, model='whisper-1'))
...
transcription_chunks = pxt.create_view(
    ...
    docs_table,
    iterator=StringSplitter.create(text=docs_table.transcription_text, separators='sentence'),
)

audio_chunks = pxt.create_view(
    'chatbot.audio_chunks',
    docs_table,
    iterator=StringSplitter.create(text=docs_table.audio_transcription_text, separators='sentence'),
)
