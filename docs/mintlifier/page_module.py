...
class ModulePageGenerator(PageBase):
    ...
    def _document_all_public(self, module: Any, module_path: str, parent_groups: List[str]) -> str:
        ...
        # Blacklist of internal classes that should never be documented
        # These are runtime/internal classes that get injected into modules
        INTERNAL_BLACKLIST = {
            'Env', 'TempStore', 'MediaStore', 'Catalog', 'StorageBackend',
            'TransactionalDirectory', 'Config', 'Client', 'Server',
            'StorageManager', 'CacheManager', 'FileCache', 'S3Client',
            'TableVersionHandle', 'SchemaObject', 'Path', 'Dir',
            'TableVersion', 'ColumnVersion', 'FunctionVersion'
        }
