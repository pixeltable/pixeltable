---
title: "Video"
description: "Build a video search pipeline with Pixeltable"
icon: "video"
---

# Building a Video Search Pipeline

Pixeltable lets you build video search pipelines in two phases:
1. Define your processing pipeline (once)
...
<CardGroup cols={1}>
  <Card title="Automatic Processing" icon="gears">
    Pipeline handles transcription and embedding automatically:
    ```python
    audio_t.add_computed_column(
        transcription=whisper.transcribe(
            audio=audio_t.audio_file
...
<CardGroup cols={2}>
  ...
  <Card title="Performance Tuning" icon="gauge-high" href="/docs/audio/performance">
    Optimize your audio search pipeline for speed and accuracy
  </Card>
</CardGroup>