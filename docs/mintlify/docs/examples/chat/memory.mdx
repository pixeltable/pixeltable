...
<Steps>
  <Step title="Persistent Memory" icon="database">
    Every conversation is automatically stored and versioned:
    ```python
    # Query past conversations
    memory = pxt.get_table("chatbot.memory")
    history = memory.collect().to_pandas()
    ```
  </Step>
  ...
  <Step title="Production Ready" icon="rocket">
    ...
    chat_session.insert([
        {"user_message": msg, "timestamp": datetime.now()} 
        for msg in messages
    ])
    ```
  </Step>
...
- [Build a Multimodal Chat](/docs/documentation/flows/chat/multimodal)
- [Add Embedding Search](/docs/documentation/flows/search/semantic)
