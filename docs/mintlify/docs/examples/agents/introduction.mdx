...
This differs from workflows, which are systems where LLMs and tools are orchestrated through predefined code paths. See [Workflows](/examples/agents/workflows) for more details.
...
class Agent:
    ...
    def __init__(self, name: str, system_prompt: str, model: str = "gpt-4o-mini", 
                 tools: Optional[pxt.tools] = None, reset: bool = False):
        self.name = name
