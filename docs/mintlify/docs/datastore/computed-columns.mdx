...
description: 'Learn how to add and manage computed columns with smart incremental updates'
icon: 'calculator'
---

<Note>
Computed columns are one of Pixeltable's most powerful features, combining automatic calculations with smart incremental updates. Think of them as your data pipeline automated.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Example Pipeline" icon="diagram-project">
    ```python
...
<Tabs>
  <Tab title="Basic Pipeline">
    Create a simple calculation pipeline:
    ```python
...
<CardGroup cols={2}>
  ...
  <Card title="Type Safety" icon="shield-check">
    Computed columns maintain type safety throughout the pipeline
  </Card>
...
<AccordionGroup>
  <Accordion title="AI Pipelines" icon="robot">
    Similar to our [chatbot example](/docs/documentation/chat/basic), you can create AI pipelines:
    ```python
...
<Tip>
Think of computed columns as building blocks for your data pipeline. Each column can depend on others, creating a powerful and maintainable data transformation workflow.
</Tip>