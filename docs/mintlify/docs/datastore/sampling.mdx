...
country = random.choices(['USA', 'Canada', 'Mexico'], k=1000)
continent = random.choices(['North America', 'South America'], k=1000)
pop_2023 = random.choices(range(0, 1000000), k=1000)
data = []
...
# Get a random sample of 1,000 countries
sample_df = pop_data.sample(n=1000, seed=42)
sample_df.collect()
```
