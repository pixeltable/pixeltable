...
<CardGroup cols={1}>
  ...
  <Card title="Import from Source" icon="file-import">
    ...
    # Or create from a polars DataFrame
    import polars as pl
    df = pl.DataFrame({
        'cca3': ['FRA', 'DEU', 'ITA'],
        'country': ['France', 'Germany', 'Italy'],
    ...
    table = pxt.create_table('europe_population_polars', source=df)
    ...
    # Or insert from a polars DataFrame
    new_df = pl.DataFrame({
        'cca3': ['NOR', 'SWE', 'DNK'],
        'country': ['Norway', 'Sweden', 'Denmark'],
        'continent': ['Europe', 'Europe', 'Europe'],
        'pop_2023': [5_400_000, 10_400_000, 5_800_000]
    })
    table.insert(new_df)
    ```
