...
4. **Build Data Pipelines**: When you need to chain multiple data transformations together
5. **Save Storage**: When you want to compute data on demand rather than storing it permanently

<Note>
Choose views over tables when your data is derived from other tables and needs to stay synchronized with its source. Use regular tables when you need to store original data or when the computation cost of deriving data on demand is too high.
</Note>

## Phase 1: Definition

Create `schema.py` to define your view structure:
...
documents = pxt.create_table(
    ...
    schema_or_df={"document": pxt.Document}
)
...
<CardGroup cols={2}>
  <Card title="View Design" icon="paintbrush">
    - Use views for data transformations
    - Keep transformation logic in the view definition
    - Design views to be reusable
  </Card>
  
  <Card title="Performance" icon="gauge">
    - Consider chunking size for document views
    - Use appropriate indexes on base tables
    - Monitor view computation time
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="Document Processing" icon="file-lines">
    ...
    # Add embeddings
    chunks.add_computed_column(
        embedding=model(chunks.text)
    )
    ```
  </Accordion>

  <Accordion title="Filtered Views" icon="filter">
    ```python
    # Create view of recent documents
    recent_docs = pxt.create_view(
        'docs.recent',
        documents.where(
            documents.timestamp >= (datetime.now() - timedelta(days=30))
        )
    )
    ```
  </Accordion>
...
<CardGroup cols={3}>
  ...
  <Card title="Pipeline Integration" icon="diagram-project">
    Views can be part of larger data pipelines
  </Card>
</CardGroup>
