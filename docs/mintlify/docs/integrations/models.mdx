...
<CardGroup cols={2}>
  <Card
    ...
    icon="face-smile"
    href="https://github.com/pixeltable/pixeltable/blob/main/docs/notebooks/integrations/working-with-hugging-face.ipynb"
  >
  ...
  <Card
    title="Replicate"
    icon="clone"
    href="https://github.com/pixeltable/pixeltable/blob/main/docs/notebooks/integrations/working-with-replicate.ipynb"
  >
...
Pixeltable provides seamless integration with Hugging Face's transformers library through built-in UDFs. These functions allow you to use state-of-the-art models directly in your data pipelines.
...
# For text embedding
table['text_embedding'] = clip(
    table.text_column,
    model_id='openai/clip-vit-base-patch32'
)

# For image embedding
table['image_embedding'] = clip(
    table.image_column,
    model_id='openai/clip-vit-base-patch32'
)
```
...
table['similarity_score'] = cross_encoder(
    table.sentence1,
    table.sentence2,
...
table['detections'] = detr_for_object_detection(
    table.image,
...
table['coco_format'] = detr_to_coco(table.image, table.detections)
```
...
table['embeddings'] = sentence_transformer(
    table.text,
...
# Basic transcription
table['transcript'] = speech2text_for_conditional_generation(
    table.audio,
    model_id='facebook/s2t-small-librispeech-asr'
)

# Multilingual translation
table['translation'] = speech2text_for_conditional_generation(
    table.audio,
...
table['classifications'] = vit_for_image_classification(
    table.image,
...
<AccordionGroup>
  <Accordion title="Computed Columns">
    ...
    table['image_features'] = clip(table.image, model_id='openai/clip-vit-base-patch32')
    table['classifications'] = vit_for_image_classification(table.image, model_id='google/vit-base-patch16-224')
    ```
  </Accordion>
