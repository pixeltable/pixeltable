...
Add a computed column that applies the model `openai/clip-vit-base-patch32` to an existing Pixeltable column `tbl.text` of the table `tbl`: The same would work with an image column `tbl.image` in place of `tbl.text`. 
```python
...
```python
cross_encoder(sentences1: String,
     sentences2: String,
     model_id: String) -> Float
```
...
```python
cross_encoder_list(sentence1: String,
     sentences2: Json,
     model_id: String) -> Json
```
...
```python
detr_for_object_detection(image: Image,
     model_id: String,
     threshold: Float,
...
```python
sentence_transformer(sentence: String,
     model_id: String,
     normalize_embeddings: Bool) -> Array[(None,),
     Float]
```
...
```python
sentence_transformer_list(sentences: Json,
     model_id: String,
     normalize_embeddings: Bool) -> Json
```
...
```python
speech2text_for_conditional_generation(audio: Audio,
     model_id: String,
...
Add a computed column that applies the model `facebook/s2t-small-librispeech-asr` to an existing Pixeltable column `audio` of the table `tbl`: Add a computed column that applies the model `facebook/s2t-medium-mustc-multilingual-st` to an existing Pixeltable column `audio` of the table `tbl`, translating the audio to French: 
...
```python
vit_for_image_classification(image: Image,
     model_id: String,
     top_k: Int) -> Json
```
