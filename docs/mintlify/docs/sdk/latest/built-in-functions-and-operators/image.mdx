...
```python
b64_encode(img: Image, image_format: String) -> String
```
...
```python
entropy(self: Image, mask: Optional[Image], extrema: Optional[Json]) -> Float
```
...
```python
getbbox(self: Image, alpha_only: Bool) -> Json
```
...
```python
getcolors(self: Image, maxcolors: Int) -> Json
```
...
```python
getpalette(self: Image, mode: Optional[String]) -> Json
```
...
```python
histogram(self: Image, mask: Optional[Image], extrema: Optional[Json]) -> Json
```
...
```python
point(self: Image, lut: Json, mode: Optional[String]) -> Image
```
...
```python
quantize(self: Image, colors: Int, method: Optional[Int], kmeans: Int, palette: Optional[Int], dither: Int) -> Image
```
...
```python
reduce(self: Image, factor: Int, box: Optional[Json]) -> Image
```
