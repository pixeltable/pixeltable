...
embeddings(input: String,
     model: String,
     model_kwargs: Optional[Json]) -> Array[(None,),
     Float]
```
...
Add a computed column that applies the model `text-embedding-3-small` to an existing Pixeltable column `tbl.text` of the table `tbl`: Add an embedding index to an existing column `text`, using the model `text-embedding-3-small`: 
...
speech(input: String,
     model: String,
     voice: String,
     model_kwargs: Optional[Json]) -> Audio
```
...
vision(prompt: String,
     image: Image,
     model: String,
     model_kwargs: Optional[Json]) -> String
```
