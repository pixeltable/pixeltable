...
Create a snapshot `my_snapshot` of a table `my_table`: Create a snapshot `my_snapshot` of a view `my_view` with additional int column `col3`, if `my_snapshot` does not already exist: 
...
Create a table with an int and a string column: Create a table from a select statement over an existing table `orig_table` (this will create a new table containing the exact contents of the query): 
...
Create a view `my_view` of an existing table `my_table`, filtering on rows where `col1` is greater than 10: Create a view `my_view` of an existing table `my_table`, filtering on rows where `col1` is greater than 10, and if it not already exist. Otherwise, get the existing view named `my_view`: 
...
Remove a directory, if it exists and is empty: Remove a subdirectory: 
...
Drop a table by its fully qualified name: Drop a table by its handle: 
...
Get contents of top-level directory: Get contents of 'dir1': 
...
Get handle for a table in the top-level directory: For a table in a subdirectory: 
...
Move a table to a different directory: Rename a table: 
