...
- **`source`** (*str | catalog.Table*): Path to the source table, or (if the source table is a local table) a handle to the source table.

- **`bucket_name`** (*str | None*): The name of the pixeltable cloud-registered bucket to use to store replica's data. If no `bucket_name` is provided, the default Pixeltable storage bucket will be used.
...
- *Any*: Pandas DataFrame with columns 'Path', 'Name', 'Parameters', 'Return Type', 'Is Agg', 'Library'
...
- *Any*: A list of [`Table`](pixeltable.Table) paths.
...
- *Any*: A list of dictionaries containing data from the table, one per row that matches the input arguments. If there are no matching rows, an empty list will be returned.
...
- *Any*: A `Tool` instance that can be passed to an LLM tool-calling API.
...
- *Any*: A `Tools` instance that can be passed to an LLM tool-calling API or invoked to generate tool results.
