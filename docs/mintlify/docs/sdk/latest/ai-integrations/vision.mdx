...
```python
draw_bounding_boxes(img: Image, boxes: Json, labels: Optional[Json], color: Optional[String], box_colors: Optional[Json], fill: Bool, width: Int, font: Optional[String], font_size: Optional[Int]) -> Image
```
...
- *Any*: The image with bounding boxes drawn on it.
...
```python
eval_detections(pred_bboxes: Json, pred_labels: Json, pred_scores: Json, gt_bboxes: Json, gt_labels: Json, min_iou: Float) -> Json
```
...
- *Any*: A list of dictionaries, one per label class, with the following structure:
