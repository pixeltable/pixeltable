...
```python
pixeltable.functions.gemini.generate_videos(prompt: Optional[String], image: Optional[Image], model: String, config: Optional[Json]) -> Video
```
...
<ParamField path="prompt" type="Optional[String]" required>
  A text description of the videos to generate.


</ParamField>

<ParamField path="image" type="Optional[Image]" required>
