...
```python
chat(messages: Json, model: String, tools: Optional[Json], format: Optional[String], options: Optional[Json]) -> Json
```
...
```python
embed(input: String, model: String, truncate: Bool, options: Optional[Json]) -> Array[(None,), Float]
```
...
```python
generate(prompt: String, model: String, suffix: String, system: String, template: String, context: Optional[Json], raw: Bool, format: Optional[String], options: Optional[Json]) -> Json
```
