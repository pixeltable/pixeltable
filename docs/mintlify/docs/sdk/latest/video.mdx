...
clip(video: Video,
     ...
     duration: Optional[Float]) -> Optional[Video]
```
...
- *Any*: New video containing only the specified time range or None if start_time is beyond the end of the video.
...
- *Any*: A new video containing the merged videos.
...
extract_frame(video: Video,
     timestamp: Float) -> Optional[Image]
```
...
- *Any*: The extracted frame as a PIL Image, or None if the timestamp is beyond the video duration.
...
Extract the first frame from each video in the `video` column of the table `tbl`: Extract a frame close to the end of each video in the `video` column of the table `tbl`: 
...
- *Any*: The duration in seconds, or None if the duration cannot be determined.
...
overlay_text(video: Video,
     ...
     box_color: String,
     box_opacity: Float,
     box_border: Optional[Json]) -> Video
```
...
- *Any*: A new video with the text overlay applied.
...
Add a simple text overlay to videos in a table: Add a YouTube-style caption: 
...
- *Any*: List of file paths for the generated video segments.
...
Split a video at 1 minute intervals Split video into two parts at the midpoint: 
