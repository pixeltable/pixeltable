...
Set default destinations for all media columns in your `config.toml` (see [Configuration](/overview/configuration) for details):
...
<Tabs>
  ...
  <Tab title="Example">
    ...
    # Or set destination per computed column
    t.add_computed_column(
        frames=pxt.iterators.FrameIterator.create(t.video, fps=1),
...
<Tabs>
  ...
  <Tab title="Example">
    ...
    # Or set destination per computed column
    t.add_computed_column(
        chunks=pxt.iterators.DocumentSplitter.create(t.document),
...
<Tabs>
  ...
  <Tab title="Example">
    ...
    t.add_computed_column(
        segments=pxt.iterators.AudioSplitter.create(t.audio, chunk_duration_sec=30),
...
<Card title="Configuration Reference" icon="gear" href="/overview/configuration">
