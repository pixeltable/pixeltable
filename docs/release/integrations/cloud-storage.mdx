...
You can set default destinations for all media columns in three places, listed here from highest to lowest precedence (see [Configuration](/platform/configuration#per-database-settings) for details).

**The database entry** in `pixeltable.toml`, for one database (see the [`[[pixeltable.database]]` reference](/platform/cli#cloud-configuration-reference)). Leave `name` off for the local database, or set `name = 'pxt://org:db'` for a hosted one:
