...
The key can also be `api_key` in the `[pixeltable]` section of the Pixeltable config file; `PIXELTABLE_API_KEY` wins when both are set, and either one outranks a `pxt login` session ([Signing in](/platform/cli#signing-in)). Hosted pods get their own worker key from the platform; you do not set it.
...
Or the same dest keys in `~/.pixeltable/config.toml`:
