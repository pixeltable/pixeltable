...
### Default destinations per database

Set default destinations for all media columns of a database in its `[[pixeltable.database]]` entry in `pixeltable.toml` (see [Configuration](/platform/configuration#per-database-settings) for details). The entry with no `name` configures the local database; an entry named `pxt://org:db` configures that hosted database.
...
Or via environment variables, which override the entry:
...
<Note>
The `destination` parameter only applies to **stored computed columns**. For input columns, use the database's `db_input_media_dest` setting.
</Note>
...
2. **Database default**: `db_input_media_dest` / `db_output_media_dest` in the `[[pixeltable.database]]` entry, or the `PIXELTABLE_INPUT_MEDIA_DEST` / `PIXELTABLE_OUTPUT_MEDIA_DEST` env vars that override it
