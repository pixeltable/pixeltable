...
A default destination applies to every media column that names none. It can be set in three places, listed here from highest to lowest precedence (see [Configuration](/platform/configuration#per-database-settings) for details).
...
**The database entry** in `pixeltable.toml`, for one database. The entry with no `name` configures the local database; an entry named `pxt://org:db` configures that hosted database:
...
**The global config** in `~/.pixeltable/config.toml`, for every database on the installation (not the API key):
...
2. **Environment variable**: `PIXELTABLE_INPUT_MEDIA_DEST` / `PIXELTABLE_OUTPUT_MEDIA_DEST`
3. **Database entry**: `db_input_media_dest` / `db_output_media_dest` in `[[pixeltable.database]]`
4. **Global config**: `input_media_dest` / `output_media_dest` under `[pixeltable]` in `config.toml`
5. **Hosted Cloud process**: `pxtfs://org:db/home` when the database is running in Cloud
6. **Local process**: local disk
