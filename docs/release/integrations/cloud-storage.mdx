...
| Local Pixeltable against that Cloud home bucket | `PIXELTABLE_API_KEY` (API Keys, then `.env` or export) plus `PIXELTABLE_INPUT_MEDIA_DEST` / `PIXELTABLE_OUTPUT_MEDIA_DEST` pointing at the `pxtfs://` URI. |
...
Create the key under **API Keys** in the dashboard. Put it in `.env` or export it. Provider keys (`AWS_ACCESS_KEY_ID`, `OPENAI_API_KEY`, …) go under **Secrets** or `pxt secret set`.
