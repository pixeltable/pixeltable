...
**The database entry** in `pixeltable.toml`, for one database (see the [database entry reference](/platform/cli#cloud-configuration-reference)). Leave `name` off for the local database, or set `name = 'pxt://org:db'` for a hosted one:
