...
<Note>
The `destination` parameter only applies to **stored computed columns**. For input columns, use the database's `input_media_dest` setting.
</Note>
...
2. **Database default**: `input_media_dest` / `output_media_dest` in the `[[pixeltable.database]]` entry, or the `PIXELTABLE_INPUT_MEDIA_DEST` / `PIXELTABLE_OUTPUT_MEDIA_DEST` env vars that override it
