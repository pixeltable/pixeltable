...
2. **Database default**: `input_media_dest` for input columns, `output_media_dest` for computed columns
...
A Pixeltable Cloud database comes with a managed storage bucket. No bucket creation, no credentials file, no cloud provider account. Cloud is in Limited Beta: email [contact@pixeltable.com](mailto:contact@pixeltable.com) to get an account.
...
<Tabs>
  ...
  <Tab title="Authentication">
    ...
    Pixeltable automatically fetches and refreshes temporary credentials from the Pixeltable cloud management API.
  </Tab>
</Tabs>
