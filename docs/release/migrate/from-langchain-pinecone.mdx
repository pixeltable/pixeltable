...
- **Survives restarts.** Restart your notebook, your script, or your server — all data, chunks, embeddings, and LLM responses are still there. No need to re-index.
- **Test before you commit.** Preview any transformation on a sample before adding it as a column: `qa.select(qa.question, summary=summarize(qa.question)).head(3)`. Nothing is stored until you call `add_computed_column()`.
- **Swap providers in one line.** The same pipeline works with Anthropic, Gemini, HuggingFace, Ollama, and [20+ other providers](/integrations/frameworks). Change `openai.chat_completions` to `anthropic.messages` — the rest stays the same.
