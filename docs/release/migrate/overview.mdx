...
Across all of these, the shift is the same:
...
# Insert a question — context retrieval and answer generation happen automatically
qa.insert([{'question': 'How do I reset my password?'}])

# Test a transformation before committing it to the table
qa.select(qa.question, qa.answer).head(3)
```

Every intermediate result is stored, versioned, and queryable. Nothing is recomputed unless the source data changes. And the same patterns work with any provider — swap `openai` for `anthropic`, `gemini`, or `huggingface` and the pipeline stays the same.
