...
<Note>**Related use case:** [Multimodal backend](/use-cases/multimodal-backend)</Note>
...
|---|---|
| Postgres / MySQL for metadata | [`pxt.create_table()`](/tutorials/tables-and-data-operations) with typed columns; load existing tables with [`import_sql()`](/howto/cookbooks/data/data-export-sql) |
...
| LangChain `DocumentLoader` | `insert()`, [`import_sql()`](/howto/cookbooks/data/data-export-sql), [`import_csv()`](/howto/cookbooks/data/data-import-csv), [import from S3](/integrations/cloud-storage) |
...
<CardGroup cols={2}>
  <Card title="Multimodal backend" icon="microchip" href="/use-cases/multimodal-backend">
