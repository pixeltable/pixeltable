...
- **Multimodal in one row.** Structured data (metadata, labels, scores) and unstructured data (images, embeddings, LLM responses) live in the same table row. No more scattered JSON files, Postgres tables, and S3 paths that you have to keep in sync.
- **Test before you commit.** Preview any transformation on a sample before adding it as a computed column: `t.select(t.text, summary=summarize(t.text)).head(3)`. Nothing is stored until you call `add_computed_column()`.
- **Swap providers in one line.** The same pipeline patterns work with OpenAI, Anthropic, Gemini, HuggingFace, Ollama, and [20+ other providers](/integrations/frameworks). Change the model, not the architecture.
...
<Tabs>
  ...
  <Tab title="Pixeltable">
    ...
    images.add_computed_column(caption=vision(
