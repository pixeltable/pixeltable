...
<Steps>
  ...
  <Step title="Upload the project, create tables, and start the service">
    ...
    `pxt db update` uploads the project and builds the image. It does not insert rows. `pxt schema update` creates its tables. `pxt service update` starts the HTTP service. `pxt service list` prints its URL and routes.
  </Step>
