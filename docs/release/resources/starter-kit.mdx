...
The kit follows Declare, Experiment, Serve: `pxt schema update`, then `pxt service update`, then insert, `/ask`, or `pxt dashboard`.
...
Same file against Cloud: `pxt db update pxt://org:mydb`, then `pxt schema update app.py pxt://org:mydb`, then `pxt service update app.py pxt://org:mydb`.
[Deploy to Pixeltable Cloud](/howto/deployment/cloud). `pxt service run` is local only.
