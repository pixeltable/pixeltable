---
title: 'Starter kit'
description: 'uvx pixeltable-new copies a starter app. pxt schema update creates its tables. pxt service update starts HTTP.'
---
...
First create the tables (`pxt schema update`), then start HTTP (`pxt service update`), then try the app by inserting rows, calling `/ask`, or opening the dashboard.
...
If you do not want HTTP, see [Self-hosting](/howto/deployment/overview) for the insert-and-export path.
...
Pixeltable Cloud lists these starter templates from the kit's
...
[Deploy to Pixeltable Cloud](/howto/deployment/cloud). `pxt service run` starts a service in your terminal and cannot target Cloud; use `pxt service update` for hosted HTTP.
