...
description: 'pxt schema update creates tables. pxt service update starts endpoints. diff shows pending changes.'
---
...
One Python file declares tables and, if you want endpoints, routes. `pxt schema update` creates the tables. `pxt service update` starts the endpoints on an assigned port. The last argument names a catalog such as `my_app`, or a Cloud URI such as `pxt://org:db`. It does not create a folder beside `app.py`.
...
<CardGroup cols={3}>
  ...
  <Card title="Database" icon="database" href="#database">
    A local catalog name, or a `pxt://` Cloud URI.
  </Card>
</CardGroup>
...
If you change only a computed column's expression, `pxt schema update` accepts the change when the column's type and other properties stay the same. Existing stored values are unchanged. Run `pxt recompute my_app/docs COLUMN -f` to update those rows and dependent columns; use `-n` first to check the row count.
...
<Tabs>
  ...
  <Tab title="Cloud">
    ...
    Add this hosted entry to `pixeltable.toml`, keeping the local entry that `pxt init` wrote. A new organization gets the `main` database when you run `pxt org create`:
    ...
    If `pxt init` wrote to `pyproject.toml` instead, use `[[tool.pixeltable.database]]` for the hosted entry. Then run:
    ...
    [Pixeltable Cloud](/cloud#first-deployment) shows the full setup. `pxt service run` only starts endpoints in your local terminal.
  </Tab>
</Tabs>
