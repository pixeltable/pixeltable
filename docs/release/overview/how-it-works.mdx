...
<Tabs>
  ...
  <Tab title="Cloud">
    Sign in with `pxt login`. Use the same `app.py` with a `pxt://` URL as the last argument after `pxt db update`. [Pixeltable Cloud](/cloud). `pxt service run` only starts endpoints in your local terminal.
