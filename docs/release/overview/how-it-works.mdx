...
description: 'pxt schema creates tables. pxt service starts HTTP. diff shows what would change.'
---
...
One Python file declares tables and, if you want HTTP, routes. `pxt schema update` creates the tables. `pxt service update` starts HTTP on an assigned port. The last argument is a namespace name such as `my_app`, or a Cloud URL such as `pxt://org:db`. It does not create a folder beside `app.py`.
...
Set `base=Docs` to make a view of `Docs`. Add an iterator when each input row should become many output rows, such as frames or chunks. Indexes belong on the model (`__indexes__` or an `EmbeddingIndex` assignment). Do not call `add_embedding_index()` after `pxt schema update`.
...
<Tabs>
  <Tab title="Local">
    No Cloud account. Follow the [Quickstart](/overview/quick-start) on this machine. For which process should listen, see [Self-hosting](/howto/deployment/overview).
  ...
  <Tab title="Cloud">
    Use the same `app.py` with a `pxt://` URL as the last argument after `pxt db update`. [Deploy to Pixeltable Cloud](/howto/deployment/cloud). `pxt service run` only starts HTTP in your local terminal.
...
<CardGroup cols={3}>
  ...
  <Card title="HTTP serving" icon="globe" href="/howto/deployment/serving">
    Route API, file uploads, background jobs.
  </Card>
</CardGroup>
