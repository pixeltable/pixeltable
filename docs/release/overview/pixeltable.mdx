...
Locally, create those tables with `pxt schema update`. Start the endpoints with `pxt service update`. On [Pixeltable Cloud](/cloud), sign in with `pxt login` or use an API key, then run `pxt db update` with the same `app.py`. That command creates or updates the hosted database. It does not insert rows or start app endpoints.
