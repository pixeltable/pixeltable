...
description: 'Put tables and routes in app.py. Computed columns run when rows are inserted.'
---
...
Put `TableModel` tables, computed columns, indexes, and routes in `app.py`. Run `pxt schema update` to create the tables, then insert a row. Pixeltable computes assigned columns and updates indexes during the insert. Run `pxt service update` when you want HTTP endpoints.
...
Locally, create those tables with `pxt schema update`. Start the endpoints with `pxt service update`. On [Pixeltable Cloud](/howto/deployment/cloud), sign in with `pxt login` or use an API key, then run `pxt db update` with the same `app.py`. That command creates or updates the hosted database. It does not insert rows or start app endpoints.
