...
**Store multimodal data in tables, define transformations as computed columns, and query everything together.** Pixeltable handles orchestration, caching, incremental updates, and model execution automatically.

<PipelineDiagram />

## The Problem

Today's AI workflows cobble together disconnected tools—and pay the price.

<Tabs>
  <Tab title="Fragmented Storage">
    **What you get:** Videos in S3, embeddings in Pinecone, metadata in Postgres. Data loaded into memory, exported to file formats that don't support media. Manual tracking of what lives where.
    ...
    **What you miss:**
    ...
    - **Persistence:** Work happens in memory → doesn't map to database schemas
    - **OLTP:** Built for batch → can't serve user-facing requests
  </Tab>
  <Tab title="DIY Orchestration">
    **What you get:** Custom code wiring tools together. Cron jobs and Python scripts for every step. Manually handling rate limits, retries, chasing API errors. Wild goose chase when requirements change.
    
    **What you miss:**
    - **Dependency tracking:** Transforms in scripts → hard to trace what depends on what
    - **Performance:** Hard to parallelize API calls → low throughput
    - **Failure handling:** Something fails → rerun everything
    - **Consistency:** Different models for indexing vs querying → contaminated index
  </Tab>
</Tabs>

**Pixeltable solves both.** One system for storage, orchestration, and retrieval—with transactions, incremental updates, and automatic dependency tracking built in.
