...
**Why Pixeltable is LLM-friendly:**
- **Simple primitives:** Tables, computed columns, views, and indexes compose predictably
- **Declarative:** No control flow to debug—just define schemas and transformations
- **Idempotent:** `if_exists='ignore'` makes generated code safe to re-run
- **Self-documenting:** Table schemas and column dependencies are explicit

```python
# LLMs can easily generate this pattern
t = pxt.create_table('myapp.docs', {'text': pxt.String}, if_exists='ignore')
t.add_computed_column(embedding=embed(t.text), if_exists='ignore')
t.add_embedding_index('embedding', if_not_exists=True)
```
