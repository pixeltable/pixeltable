...
pxt service logs pxt://myorg:mydb/services/ingest   # what a hosted service logged
...
A hosted service's log is read with `pxt service logs`, and the database's own pod with `pxt db logs`:
...
The log holds what the serving process logged, requests included, merged by time with what it wrote to its
console, which is where a service that failed to start left its traceback. Health probes are left out unless
`--include-health` is given. The log outlives the pod, so a failed deploy can be read after its pod is gone, and
a line reaches it a few seconds after it is written.
...
| `pxt service logs URI` | What a hosted service logged; see [Hosted targets](#hosted-targets) |
...
| `--since`, `--tail`, `--include-health` | `logs` | How far back, how many lines, whether to keep health probes; see [Hosted targets](#hosted-targets) |
