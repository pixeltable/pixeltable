...
pxt db update pxt://myorg:mydb   # apply it
```
...
```bash
pxt db update pxt://acme:main             # send this project there and build its image
...
so an edit to a source file is sent in seconds, and only a dependency change costs an image build. Exit status is 0 in agreement and 2 with changes pending; nothing is built, resized or set.
