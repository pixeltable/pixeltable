...
A service's log is read with `pxt service logs`, addressing the service the way `stop` does, and a hosted
database's own pod with `pxt db logs`:
...
A hosted log holds what the serving process logged, requests included, merged by time with what it wrote to its
...
a line reaches it a few seconds after it is written. For a service running on this machine the log is a file
there, and the command prints its path; the options apply to hosted services.
