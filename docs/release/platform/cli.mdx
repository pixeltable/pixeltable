...
Defines which project files the database gets, what its image holds, and what it runs on.
...
`update` applies a new image if dependencies changed, then uploaded project files if sources changed, then one resize for any changed CPU, memory, disk, or worker counts. Set secrets with `pxt secret set`.
