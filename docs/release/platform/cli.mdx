...
`pxt schema` and `pxt service` read a Python file, and the tables they create refer back to the udfs that file calls. A reference is a module path, so the file has to belong to a project: a directory holding a `pixeltable.toml`, or a `pyproject.toml` with a `[tool.pixeltable]` section. `pxt init` establishes one in the current directory.

```bash
cd ~/proj
pxt init                             # writes pixeltable.toml, marking ~/proj as the project root
```

Module paths are relative to the root, and every directory from the root down to a file becomes one component of that file's path, so each of those directory names has to be a Python identifier:

```
~/proj/pixeltable.toml               # marks the root
...
A command given a file outside any project is refused, and says so. `pxt schema check` and `pxt service check` validate a file on its own -- it imports without touching the catalog, it declares what the verb needs, and the udfs its columns call are named by paths another process can resolve:
...
Read by `pxt db update-runtime`, from `pixeltable.toml` in the current directory if that file declares the section. Defines which local files to package into the runtime image.
