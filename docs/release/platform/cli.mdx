...
pxt key list                     # your keys, and the org's runtime keys
```
...
`pxt login` prints a code and opens a browser. Approve it there — signing in, or signing up if you
have no account — and the session lands on this machine. It uses the OAuth device grant, so nothing
listens on a port and the browser need not be on this machine: this works over SSH.

```bash
pxt whoami
you@example.com on https://internal-api.pixeltable.com — sign in again in 46m
```

A sign-in lasts an hour, counted from `pxt login` and not reset by renewal, so a session left on a
laptop stops working the same day. Within that hour it renews silently, and a command already
running is never cut off partway.

```bash
pxt logout
```

`pxt logout` forgets this device's cached session and signs the browser out as well. Both, because
a browser that is still signed in confirms the next code without naming an account — so signing out
of one and not the other is how you end up logged in as someone you did not choose. It signs that
browser out of the dashboard too, and of every other environment sharing the same sign-in.

`PIXELTABLE_CREDENTIALS` names the session file to use, if you need one other than the default —
a test run, which gets its own Pixeltable home, reads a real sign-in that way.
...
and every hosted command needs one, so `pxt login` says so rather than letting the next command
fail as an authorization error.
...
Creates the organization, its first database and its storage in one call. The slug is the namespace
`pxt://acme:main` refers to, and it must be unique across Pixeltable.

Creating one does not move you into it. Name the organization in the URI — `pxt://acme:main` — and
`pxt org list` shows the ones you can reach.
...
granted, so a service you deploy carries no more authority than its job needs.

A grant is a verb on a resource, never a bare resource: naming a service does not say whether the
holder may call it or reconfigure it, and those are not the same permission.
...
Cycle the pods onto the image and project they already run, without rebuilding or uploading anything. Use it
...
internal id, so anything still holding the old database's credentials reaches nothing. Treat a
...
pxt key create NAME --grant VERB:URI      # a scoped key that belongs to the org; repeatable
pxt key list                              # your keys, and the org's runtime keys
...
already had. Revoking a grant the key never held is an error rather than a no-op — a typo there would
otherwise read as a completed revocation. It applies only to a key that has grants: one that acts as
you has nothing to change.

Names are unique per organization across both shapes, so `pxt key delete NAME` never has two keys to
choose from.
...
A running database holds the values it started with. `pxt db restart`, or `pxt service restart` for one service, picks up a change; a database or service started afterwards reads it on the way up.

<Warning>
A `[[pixeltable.database]]` entry no longer accepts `secrets`. A project config that still carries
