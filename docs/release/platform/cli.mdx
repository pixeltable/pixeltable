...
current project, or every local service when the working directory is outside a project. `TARGET/NAME`
stops the service under `TARGET`, including one in another project, and `/NAME` stops the service at the
...
```bash
pxt service stop ingest                  # the service named ingest in the current project
pxt service stop my_dir/ingest           # the service of that name under my_dir
pxt service stop /ingest                 # the service of that name at the catalog root
pxt service stop pxt://acme:main/ingest  # the one in a hosted database
```

A name that matches no service, or matches one that is already stopped, is reported as `skipped` with
...
| `pxt service stop NAME...` | Stop the named services; a bare name matches within the current project |
