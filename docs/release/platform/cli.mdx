...
| **Interactive** | `shell` |
| **Serving** | `service` |
| **Cloud** | `login`, `logout`, `whoami`, `key`, `db`, `secret`, `org` |
...
| `--json` | Inspection, query, and mutation commands (`ls`, `describe`, `columns`, `computed`, `idxs`, `history`, `status`, `config`, `rows`, `get`, `count`, `errors`, `drop`, `drop-dir`, `rename`, `mv`, `recompute`, `revert`); `schema` and `service` verbs except `example`; `login`, `whoami`, `key`, `init`, `db`, `secret`, `org`, and `daemon status` | Emit machine-readable JSON. Not accepted by `shell`, `health` (always JSON), `dashboard`, `cd`, `pwd`, `logout`, or `daemon start`/`stop`. |
| `--json-schema` | `schema diff`, `service diff`, `service list`, `db diff`, `db status` | Print the JSON Schema for the command's JSON output. |
| `-n`, `--dry-run` | Catalog mutations (`drop`, `drop-dir`, `rename`, `mv`, `recompute`, `revert`), plus `schema update`, `schema prune`, `service update`, and `service prune` | Print the intended action; don't execute. |
...
pxt recompute my_dir/my_table summary -n  # see the row count before recomputing
...
Recompute stored computed columns after changing their expression. By default, columns that depend on them are recomputed too.

```bash
pxt recompute my_dir/my_table summary -n  # show how many rows would be affected
pxt recompute my_dir/my_table summary -f  # run without a confirmation prompt
```

Use `--errors-only` for rows where one computed column failed, or `--no-cascade` to leave dependent columns unchanged. Recompute may rerun paid model or API calls.
...
| `pxt schema check SCHEMA` | Validate the application file on its own. Takes no `TARGET` and reads no catalog |
...
An application file defines one or more models on a `pxt.model_base()`. Each model becomes one table, named by `name=`. `pxt schema example` writes a file covering every construct the schema DSL supports, so you never have to start from a blank page and never have to look a construct up:
...
Some differences cannot be applied in place: a table declared where a view exists, a changed iterator, or a column whose type or properties changed. Those are reported as `UNSUPPORTED`, nothing is applied, and `update` exits `1`. Adjust the application file or the table by hand.

Changing only a computed column's expression, with its type and other properties unchanged, can be applied by `pxt schema update`. Existing stored values keep their old result. Run `pxt recompute my_app/docs title_upper -f` to refresh them and dependent columns; use `-n` first to see the affected row count. Recompute can rerun paid model or API calls.
...
`pxt schema diff --json-schema` prints the JSON Schema for this output. `pxt service diff --json-schema`, `pxt db diff --json-schema`, `pxt db status --json-schema`, and `pxt service list --json-schema` do the same for their results.
...
| `pxt service restart NAME...` | Restart named services on their current project and secrets |
| `pxt service logs NAME` | Read a hosted service log, or find its local log file |
| `pxt service list [TARGET]` | What is running at a local or hosted target, and where |
...
|---|---|---|
| `--json` | All except `example` | Emit machine-readable JSON |
...
Use `pxt service list pxt://org:db` to find hosted service URLs. After changing a hosted secret, run `pxt service restart pxt://org:db/ingest`. Use `pxt service logs pxt://org:db/ingest --since 10m --tail 50` to read its logs.

## Cloud

`pxt db` and `pxt org` manage hosted databases and organizations. Sign in with `pxt login` or use an API key. If you set `PIXELTABLE_API_KEY` or `[pixeltable].api_key` in `config.toml`, the CLI uses that key in preference to the login session. [Get an API key](/cloud#get-an-api-key) and [Configuration](/platform/configuration) show both settings.

Cloud commands except `pxt logout` accept `--json` for machine-readable output.

### Signing in

```bash
pxt login                 # confirm the displayed code in a browser
pxt whoami                # show the credential this machine uses
pxt whoami --offline      # inspect a cached session without contacting Cloud
pxt logout                # forget the cached session
```

`pxt login` opens the browser for device authorization. The session is cached and renewed when needed. `pxt whoami` reports whether Cloud accepts the current credential. A configured API key takes precedence over the session.
...
Defines the project files, dependencies, compute, media destinations, and telemetry for a local or Cloud database.

```toml
[[pixeltable.database]]
name = 'pxt://myorg:prod'           # matches the uri passed to pxt db update

# where media without an explicit destination goes (default: the database's home bucket)
...
<Warning>
`DatabaseConfig` uses `extra='forbid'`. A `[[pixeltable.database]]` entry with `secrets.openai_api_key = 'env:OPENAI_API_KEY'` fails to load with `secrets: Extra inputs are not permitted`. Delete any `secrets.*` lines and set runtime secrets with `pxt secret set pxt://myorg:prod OPENAI_API_KEY=your-provider-key` instead.
</Warning>
...
`update` applies a new image if dependencies changed, uploads project files if sources changed, and resizes once if CPU, memory, disk, or worker counts changed. Set secrets with [`pxt secret`](#pxt-secret). Media destinations and the OTLP endpoint and protocol travel with the project files, which include the project configuration.
...
Restart the database pods to pick up changed secrets.
...
Read the hosted database pod log.
...
```bash
pxt org create myorg
```

Create an organization with its first database. A `pxt login` session switches to it.
...
pxt key create my-app
pxt key list
pxt key delete my-app
```

`create` prints the key once. A key without `--grant` acts as its creator. `--grant access:pxt://org:db/services/ingest` creates a scoped runtime key; scoped keys are in preview. Use `pxt key update NAME --grant VERB:URI` or `--revoke VERB:URI` to change its grants.
...
Running processes read secrets when they start. After `pxt secret set` or `pxt secret delete`, run `pxt db restart` for the database pods and `pxt service restart` for its services.
