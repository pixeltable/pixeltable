...
| `--no-cascade` | Leave the computed columns that depend on these alone |
...
<Warning>
Recomputing evaluates the column over every row, so a udf that calls a model or an API costs real time and
money. `-n` reports the row count first, and `--errors-only` narrows the work to the rows that failed.
</Warning>
