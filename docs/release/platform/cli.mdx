...
file here"; `pxt whoami --offline` reports the cached session without asking.
...
a browser that is still signed in confirms the next code without saying which account it is for, so
signing out of one and not the other leaves you logged in as someone you did not choose. It signs that
...
in `pxt://acme:main`, and it must be unique across Pixeltable.

Creating one does not move you into it. Give the organization in the URI (`pxt://acme:main`), and
