...
| `--json` | Inspection, query, and mutation commands (`ls`, `describe`, `columns`, `computed`, `idxs`, `history`, `status`, `config`, `rows`, `get`, `count`, `errors`, `drop`, `drop-dir`, `rename`, `mv`, `recompute`, `revert`), the `schema` and `service` verbs, plus `login`, `whoami`, `db`, `secret`, `org`, `key`, and `daemon status` | Emit machine-readable JSON instead. Not accepted by `shell` (interactive REPL), `health` (always JSON), `dashboard` (URL launcher), the navigation commands (`cd`, `pwd`), or the commands that print plain status lines (`logout`, `daemon start`/`stop`). |
...
A sign-in lasts as long as Pixeltable Cloud honors it: a command that needs the session renews it
first when its token has expired, and nothing on this machine expires it on a timer. `pxt whoami`
asks whether the credential is still accepted and exits nonzero when it is not, so it answers "will
my next command work" rather than "is there a file here"; `pxt whoami --offline` reports the cached
session without asking. A key whose grants do not cover the check still counts as accepted, and
`pxt whoami` prints the refusal as a note.

```bash
pxt logout
```

`pxt logout` forgets this machine's cached session. When the session has a sign-in service session
id, it also opens the sign-in service's sign-out page for that session in your browser, and prints
the link when no browser opens. Both, because a browser that is still signed in confirms the next
code without saying which account it is for, so signing out of one and not the other leaves you
signed in as someone you did not choose. Forgetting the session needs no network: when the sign-in
service cannot be reached, `pxt logout` still signs this machine out, and warns that the browser is
still signed in.

<Note>
An **API key** takes precedence over a sign-in whenever both are present: it is the explicit
choice, and what CI runs on. Set one with `PIXELTABLE_API_KEY`, or `api_key` in the `[pixeltable]`
section of your Pixeltable config file. Unlike a sign-in, it does not expire. `pxt whoami` says
which one your commands will actually send.
</Note>

Signing in proves who you are; it does not give you an organization. A brand-new account has none,
and every hosted command needs one, so `pxt login` and `pxt whoami` print "No organization yet" with
...
key revokes it. Every member of the organization sees every key and can delete any of them, and
`pxt key list` shows who created each one.
...
`pxt db` and `pxt org` manage cloud-hosted databases and organizations. Cloud commands need a credential: a `pxt login` session or an API key ([Signing in](#signing-in)). To use an API key, create it in the Cloud dashboard ([Get an API key](/howto/deployment/cloud#get-an-api-key)) or with `pxt key create`, then export it as `PIXELTABLE_API_KEY`. The CLI reads the process environment and does not load `.env` on its own ([Configuration](/platform/configuration)).
...
Defines which project files the database gets, what goes into its image, and its compute settings.
...
`update` applies a new image if dependencies changed, then uploaded project files if sources changed, then one resize for any changed CPU, memory, disk, or worker counts so the pods restart only once. Secrets are set with [`pxt secret`](#pxt-secret) rather than from the project.
