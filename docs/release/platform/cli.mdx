...
`pxt service restart` cycles a service's pods onto the image and routes they already run, and accepts the
same service addresses as `stop`. Use it to force the service to pick up the current values of all of its referenced
secrets. Against a local target it stops the process and starts it again on the same
