...
This is the command that creates a hosted database, and the one that keeps it current afterwards. Declare the database in a `[[pixeltable.database]]` entry, then run `update` against the uri its `name` holds:
