...
| `pxt service run APP TARGET` | Serve them from this process instead, until interrupted |
...
`run` serves every service the file declares from the calling process instead, on one port, until
interrupted. That is the mode for a container entrypoint or a development loop. It has no idle outcome and
records nothing, which is why it is a verb of its own rather than a mode of `update`:
