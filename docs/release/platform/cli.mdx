...
This is the command that creates a hosted database, and the one that keeps it current afterwards. Declare the database in a `[[pixeltable.database]]` entry (see the [cloud configuration reference](#cloud-configuration-reference)), then run `update` against its uri: the first run creates the database, builds the image its environment describes and uploads the project files the pods run; every later run applies whatever has changed since.

What it applies, in this order: secrets first, since code the pods run reads them as they start; then the image, if the declared environment has moved, and the project archive, if the files have; then one resize carrying every changed number, so the pods restart once. A secret is declared as the name of an environment variable (`openai_api_key = 'env:OPENAI_API_KEY'`), never as a value.
...
Uploads the project files and builds the image from its environment, without comparing anything first. `pxt db update` does whichever of the two the project has moved on from; this one does both whatever the database already holds, which is how a build is forced after one has failed. Polls until the build completes or fails; the pods restart on the new image and the new sources.
