...
Define your routes in the Pixeltable config file and start everything with one command:
...
To keep a service definition alongside a project instead, put it in its own file and point
`PIXELTABLE_CONFIG` at that file:
...
| `--otel` | flag | | Enable OpenTelemetry instrumentation (requires `pip install 'pixeltable[otel]'`) |
...
```bash
PIXELTABLE_CONFIG=service.toml pxt serve my-service --dry-run --json
```
...
```bash
PIXELTABLE_CONFIG=service.toml pxt serve my-service --dry-run
```
...
```bash
PIXELTABLE_CONFIG=service.toml pxt serve my-service --port 9000
```
...
Cloud operations that affect a database's runtime image or service routes read their configuration from TOML.

#### `[pixeltable.clouddb]` — runtime image config
...
Read by `pxt service create` and `pxt service update`, from the Pixeltable config file (`$PIXELTABLE_HOME/config.toml`, or the file `PIXELTABLE_CONFIG` names). Each `[[pixeltable.service]]` block defines one named service; `name` must match the service name argument passed to `pxt service create`.
...
Reads `[pixeltable.clouddb]` (see the [cloud configuration reference](#cloud-configuration-reference)), packages the project files, uploads them, and triggers a CodeBuild image rebuild. Polls until the build completes or fails. Running services are restarted on the new image automatically.
...
| Argument / flag | Description |
|---|---|
| `name` | Service name; must match a `[[pixeltable.service]]` block in the Pixeltable config file |
...
Reads route config from the `[[pixeltable.service]]` block matching `name` in the Pixeltable config file. Polls until `AVAILABLE`.
...
Re-reads `[[pixeltable.service]]` from the Pixeltable config file for route config changes. Triggers a rolling restart if routes changed; polls until `AVAILABLE`.
