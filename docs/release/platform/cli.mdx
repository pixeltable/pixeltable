...
| **Interactive** | `shell` |
| **Serving** | `service` |
| **Cloud** | `login`, `logout`, `whoami`, `db`, `secret`, `org` |
...
Creating one does not move you into it — `gcloud projects create` does not change your project
either:
...
That is what decides which organization commands act on when they do not name one. `pxt login` never
changes it: signing in says who you are, not what you are working on. `pxt org list` shows the ones
you can reach.
...
#### `pxt org use acme                 # work in this org; `pxt login` never changes it
pxt org list`
