...
`pxt schema diff --json` emits the whole plan as one object: `schema_path`, `target`, `in_agreement`, `tables`, `extras`, and a `summary` with one count per resolution. Each entry in `tables` carries its `path`, `resolution`, whether it is `destructive`, and its `ops`:
