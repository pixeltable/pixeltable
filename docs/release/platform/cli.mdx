...
pxt db diff   pixeltable.toml pxt://myorg:mydb   # what 'update' would change
pxt db update pixeltable.toml pxt://myorg:mydb   # apply it
```
...
machine. The database has to hold the project's code and its tables first, so the order is:

```bash
pxt db update pixeltable.toml pxt://acme:main # build the image from this project
...
`pxt db diff` and `pxt db update` take the project configuration as their first argument -- `pixeltable.toml`, or `pyproject.toml` under `[tool.pixeltable]` -- and read the entry naming the target database.

#### `[[pixeltable.database]]` — what a project declares about one database

Defines which local files go into the image, what the database runs on, and which secrets it holds.

```toml
[[pixeltable.database]]
name = 'pxt://myorg:prod'

# what goes into the image. Glob patterns relative to the project root; everything git would not
# ignore is included by default, and the lockfile always is.
include = ["app/**", "models/**"]
...
```bash
pxt db diff pixeltable.toml pxt://myorg:mydb
pxt db diff pixeltable.toml pxt://myorg:mydb --json
```

Compares the database against the `[[pixeltable.database]]` entry naming it (see the [cloud configuration reference](#cloud-configuration-reference)): the capacity numbers, which secrets the database holds, and the two artifacts a project ships separately --
...
so an edit to a source file ships in seconds, and only a dependency change costs an image build. Reports what it did not compare rather than counting it as agreement. Exit status is 0 in agreement and 2 with changes pending; nothing is built, resized or set.

#### `pxt db update`

```bash
pxt db update pixeltable.toml pxt://myorg:mydb
pxt db update pixeltable.toml pxt://myorg:mydb -n            # print the plan and stop
pxt db update pixeltable.toml pxt://myorg:mydb --workers 2   # a flag stands in for the file
pxt db update pixeltable.toml pxt://myorg:mydb --allow-destructive
```
...
Applies the difference `diff` reports, creating the database if it does not exist: secrets first, since code the pods run reads them as they start; then the image, if the declared environment has moved, and the project archive, if the files have; then one resize carrying every changed number, so the pods restart once. A secret is declared as the name of an environment variable (`openai_api_key = 'env:OPENAI_API_KEY'`), never as a value.

#### `pxt db build-image`

```bash
pxt db build-image pxt://myorg:mydb --project-dir .
```

Stores the project's archive and builds the image from its environment, without comparing anything first. `pxt db update` is the verb that ships whichever of the two has moved; this one does both unconditionally, which a database that has never had either needs. Polls until the build completes or fails; the pods restart on the new image and the new sources.
