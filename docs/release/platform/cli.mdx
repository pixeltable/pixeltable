...
A running database or service uses the values from its startup. `pxt db restart` picks up a change for the database's tables, and `pxt service restart` for each service; a database or service started afterwards reads the current value at startup.
