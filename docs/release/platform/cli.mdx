...
and prints the link when no browser opens. Both, because a browser that is still signed in
confirms the next code without saying which account it is for, so signing out of one and not the
other leaves you signed in as someone you did not choose. Forgetting the session needs no network:
when the sign-in service cannot be reached, `pxt logout` still signs this machine out, and warns
that the browser is still signed in.
