...
```json
{
  ...
  "destructive": true,
  "ops": [
    {"kind": "add_column", "column": "author", "severity": "additive",
     "destructive": false, "description": "column 'author' will be added", "type": "String"},
    {"kind": "drop_column", "column": "body", "severity": "destructive",
     "destructive": true, "description": "column 'body' will be dropped"}
  ]
}
```

An op's `kind` is one of `add_column`, `drop_column`, `alter_column`, `add_index`, `drop_index`, or `alter_table`, and it names what it acts on (`column`, `index`, or `attribute`) plus that kind's operands. `severity` is `additive`, `destructive`, or `unsupported`; `destructive` is the boolean form of the middle case. A table with `action: "create"` carries no ops, because the create subsumes them.
...
Every path returns the plan, including the ones that refuse before reaching the daemon, so an `--allow-destructive` refusal is as machine-readable as a success: exit `3`, with the offending operations marked `refused` and the rest `skipped`. `prune` reports its drops as `drop_table` ops in a top-level `ops` array.
