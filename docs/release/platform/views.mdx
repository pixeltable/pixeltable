...
<Note>
**App vs notebook.** An application's contract is a `TableModel` in `app.py`, applied with `pxt schema update`. Notebooks and tests use `pxt.create_table()` / `pxt.create_view()`.
</Note>
