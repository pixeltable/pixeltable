...
@pxt.iterator(output_columns={'word': pxt.String, 'position': pxt.Int})
def word_iterator(text: str):
    for i, word in enumerate(text.split()):
        yield {'word': word, 'position': i}
...
```python
@pxt.iterator(
    output_columns={'frame': pxt.Image, 'timestamp': pxt.Float},
    unstored_cols=['frame']
)
def my_frame_extractor(video: pxt.Video):
