  ...
  <Tab title="audio_splitter">
    ...
    chunks_view = pxt.create_view(
        ...
        iterator=audio_splitter(
            ...
            segment_duration_sec=30.0,  # Split into 30-second chunks
            overlap_sec=2.0,          # 2-second overlap between chunks
            min_segment_duration_sec=5.0 # Drop last chunk if < 5 seconds
        )
    )
    ```

    ### Parameters
    - `segment_duration_sec` (float): Duration of each audio chunk in seconds
    - `overlap_sec` (float, default: 0.0): Overlap duration between consecutive chunks in seconds
    - `min_segment_duration_sec` (float, default: 0.0): Minimum duration threshold - the last chunk will be dropped if it's shorter than this value
