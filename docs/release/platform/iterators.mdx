...
## Split a row into chunks

Set `iterator=` on a view's `TableModel` to turn one document, video, or audio row into multiple rows. For example, `document_splitter(..., separators='sentence,token_limit', limit=300)` creates chunk rows from each document. Run `pxt schema update app.py my_app`; inserting a document then creates its chunks in the view.
