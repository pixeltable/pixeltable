...
<Note>
**App vs notebook.** Declare the table as a `TableModel` in `app.py`. Inserts, updates, `history()`, and time travel below are runtime APIs and are the same in an app or a notebook.
</Note>
...
```python
products = pxt.get_table('demo.products')
...
# Version 2: Schema changed (notebook API; in app.py this is another computed column on the model)
