...
<Note>
This page leads with a `TableModel` in `app.py`. Create those tables with `pxt schema update`. Notebooks and tests can still call `pxt.create_table()`, `add_computed_column()`, and `add_embedding_index()`.
</Note>
...
A bare type is required. `title: pxt.String` rejects a missing value. Use `pxt.String | None` when the column may be empty. A column added to a table that already has rows must be nullable, so existing rows can stay empty.

Table, column, and database names match without regard to case.
...
class Example(TableModel, name='basic_types'):
    text: pxt.String
    count: pxt.Int
    score: pxt.Float
    active: pxt.Bool
