...
Because the casing you supply is not retained, two names that differ only in case denote the same thing:
a schema with both `foo` and `Foo` is rejected, and renaming or moving something to a case variant of its own
name does nothing.

Names must also be **ASCII**, valid Python identifiers that don't start with `_`, and must avoid Python keywords
and Pixeltable's own method names (`count`, `insert`, `select`, ...). Those bans apply in every casing, so `Count`
and `Class` are rejected just as `count` and `class` are.
