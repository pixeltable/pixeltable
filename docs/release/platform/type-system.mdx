...
<Note>
**App vs notebook.** An application's contract is a `TableModel` in `app.py`, applied with `pxt schema update`. Notebooks and tests use `pxt.create_table()`, `add_computed_column()`, and `add_embedding_index()`. The examples below lead with `TableModel`; the notebook form follows.
</Note>
