...
Pixeltable provides a rich type system for structured data, media (images, video, audio, documents), and ML artifacts (arrays, embeddings).

<CardGroup cols={2}>
  <Card title="Basic Types" icon="font">
    `pxt.String`, `pxt.Int`, `pxt.Float`, `pxt.Bool`, `pxt.Timestamp`, `pxt.UUID`
  </Card>
  <Card title="Media Types" icon="image">
    `pxt.Image`, `pxt.Video`, `pxt.Audio`, `pxt.Document`
  </Card>
  <Card title="ML Types" icon="brain">
    `pxt.Array` for embeddings and tensors
  </Card>
  <Card title="Flexible Data" icon="brackets-curly">
    `pxt.Json` with optional schema validation
  </Card>
</CardGroup>

```python
# Example schema with various types
table = pxt.create_table('example.media_pipeline', {
    'text': pxt.String,
    'image': pxt.Image,
...
<Card title="Multimodal Type System" icon="shapes" href="/platform/type-system">
  Learn more about Pixeltable's type system, including type specialization, JSON schema validation, type conversion, and best practices.
</Card>
