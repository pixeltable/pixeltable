...
|---|---|
| **PostgreSQL / MySQL** | `TableModel` in `app.py` — schema is Python, applied with `pxt schema update` |
| **Pinecone / Weaviate / Qdrant** | `EmbeddingIndex` on the model — auto-maintained on insert/update/delete |
...
| Need an API with zero web code? | Yes | **Declarative Serving** (`pxt service` from an application file) |
...
| Capability | [Media processing](/use-cases/media-processing) | [Multimodal backend](/use-cases/multimodal-backend) |
|------------|-------------------|-----------------|
...
<AccordionGroup>
  ...
  <Accordion title="Architecture" icon="sitemap">
    ...
    - Declare tables, views, computed columns as `TableModel` in `app.py`
    - `pxt schema update app.py TARGET` applies them (idempotent)
...
class Sentences(
    ...
    __indexes__ = [
        pxt.EmbeddingIndex(
            text,  # type: ignore[name-defined]  # iterator output
            embedding=embed_fn,
            name='sentences_embed',
        )
    ]
```
...
<Tip>
**When to keep hand-written endpoints:** Use `@router.post()` for multi-table operations, conditional logic, or custom response shapes. `FastAPIRouter` extends `APIRouter`, so hand-written and declarative routes coexist on the same router. See [HTTP serving](/howto/deployment/serving).
</Tip>
...
### Approach 3: Declarative Serving (`pxt service`)

Generate a complete REST API from an application file. No FastAPI code, no frontend, no hand-written
endpoints: one file declares the models that name the tables and the routers that serve routes over them,
and `pxt service` runs it.
...
flowchart LR
    ...
    subgraph PXT["pxt service update"]
        API["Auto-generated FastAPI"]
        Tables["Pixeltable Tables"]
    end

    App -->|declares models and routes| PXT
    Client --> API --> Tables
```
...
<AccordionGroup>
  ...
  <Accordion title="Architecture" icon="sitemap">
    - Declare tables, views, computed columns and embedding indexes as models in `app.py`
    - Declare routes on a `FastAPIRouter` in the same file
    - `pxt schema update app.py TARGET` creates the tables; `pxt service update app.py TARGET` serves them
    - Supports insert, compute, update, delete and query routes, with `export_sql`
    - Auto-generates OpenAPI/Swagger docs
  </Accordion>

  <Accordion title="What This Provides" icon="sparkles">
    - A complete REST API from one declarative file
    - Auto-generated request/response schemas
    - Background job support for long-running inserts
    - `export_sql` for pushing rows to external databases
...
@pxt.query
def search_documents(query_text: str) -> pxt.Query:
    return Documents.where(Documents.title.contains(query_text)).select(Documents.title)


api = FastAPIRouter(name='my-service', prefix='/api')
api.add_query_route(path='/search', query=search_documents)
api.add_insert_route(Documents, path='/ingest', inputs=[Documents.title, Documents.body])
```

```bash
pxt schema update app.py pipeline
pxt service update app.py pipeline
pxt service list  # pipeline/my-service  http://127.0.0.1:49213  pid 8123  app.py
# curl -X POST http://127.0.0.1:49213/api/search -d '{"query_text": "machine learning"}'
```
...
| **[`serving/`](https://github.com/pixeltable/pixeltable-starter-kit/tree/main/serving)** | **Declarative Serving** | `pxt service` from an application file: zero-code REST API with insert, query, search, and `export_sql` routes. |
...
<CardGroup cols={2}>
  <Card title="HTTP Serving" icon="globe" href="/howto/deployment/serving">
    Expose tables and queries as HTTP endpoints with `pxt service` or `FastAPIRouter`
  </Card>
