...
| Without Pixeltable | What you'd need | With Pixeltable |
|---|---|---|
| **PostgreSQL / MySQL** | Install DB, write DDL, manage migrations, connection pooling, ORMs | `pxt.create_table('name', {'col': pxt.String, 'img': pxt.Image})` — schema is Python, persistent across sessions, versioned |
| **Pinecone / Weaviate / Qdrant** | Separate vector DB, sync embeddings manually, manage indexes, handle consistency | `table.add_embedding_index('content', string_embed=sentence_transformer.using(model_id='...'))` — one line, auto-maintained on insert/update/delete |
| **S3 client / blob management** | `boto3`, bucket config, upload/download logic, URL signing, cache management | Pixeltable's type system (`pxt.Image`, `pxt.Video`, `pxt.Audio`, `pxt.Document`) handles media transparently — reference files (local or remote) and Pixeltable manages caching. Use `destination='s3://...'` on computed columns to route generated media to cloud buckets |
| **Airflow / Prefect / Celery** | DAG definitions, worker processes, scheduling, retry logic, monitoring | Computed columns execute automatically when data is inserted or updated — no orchestrator, no workers, no DAGs. Dependency chains between computed columns resolve automatically |
| **LangChain / LlamaIndex** (RAG) | Retriever setup, chain definitions, vector store adapters, prompt templating for retrieval | `@pxt.query` for retrieval functions + `.similarity()` for semantic search + computed column chaining for the full RAG pipeline |
| **pandas / polars** (multimodal experimentation) | Separate notebook workflow, no persistence, manual parallelism, results lost between sessions, no path to production | `t.sample(5).select(t.text, summary=my_udf(t.text)).collect()` to test on a sample; `add_computed_column()` to commit to the full dataset. Automatic parallelization of API calls, built-in caching, and incremental updates — [same code from experiment to production](/howto/cookbooks/core/dev-iterative-workflow) |
| **DVC / MLflow / W&B** | Data versioning setup, experiment tracking infrastructure, artifact management, separate UI | Built-in [`history()`](/platform/version-control), [`revert()`](/platform/version-control), time travel queries (`table:N`), and [snapshots](/platform/version-control) — all automatic with zero configuration |
| **Custom retry / rate-limit / caching** | Per-provider wrapper code, Redis or Memcached, request queues, deduplication logic | Built into every [API integration](/integrations/frameworks); computed results cached automatically and only recomputed for new or changed rows |
| **Custom ETL / glue code** | Scripts wiring data extraction, transformation, loading, error handling, retries | Declarative schema — define the table, its computed columns, and the relationships. Pixeltable handles execution, caching, and incremental updates |
...
|---|---|---|
| **FFmpeg** | Install binary, subprocess calls, format conversion, frame seeking | [`extract_audio(video, format='mp3')`](/sdk/latest/video#func-extract_audio) for audio; [`frame_iterator(video, fps=1)`](/sdk/latest/video#cls-frame_iterator) for frame extraction via `pxt.create_view()` |
...
| **spaCy** | `pip install spacy`, download model, load pipeline, parse documents | [`document_splitter(doc, separators='sentence')`](/sdk/latest/document#cls-document_splitter) — spaCy runs under the hood (configurable via `spacy_model` parameter). Also supports `'heading'`, `'paragraph'`, `'page'`, `'token_limit'`, `'char_limit'` separators |
| **sentence-transformers** | Load model, tokenize, encode batches, normalize vectors | [`sentence_transformer.using(model_id='intfloat/e5-large-v2')`](/sdk/latest/huggingface#func-sentence_transformer) passed to `add_embedding_index()`. Pixeltable handles model loading, batching, and index maintenance |
| **OpenAI CLIP** | Load model, preprocess images/text differently, encode, handle multimodal alignment | [`clip.using(model_id='openai/clip-vit-base-patch32')`](/sdk/latest/huggingface#func-clip) — multimodal embedding index that accepts both image and text queries for cross-modal search |
| **OpenAI Whisper** | API key setup, audio format handling, chunking long files, parsing responses | [`openai.transcriptions(audio=table.audio_col, model='whisper-1')`](/sdk/latest/openai#func-transcriptions) as a computed column — automatic rate limiting, caching. Also supports local Whisper via [`whisper.transcribe()`](/sdk/latest/whisper) |
