...
def search_documents(query: str, limit: int = 10) -> list[SearchResult]:
    t = pxt.get_table('myapp/documents')
    sim = t.embedding.similarity(string=query)
    results = t.select(
        t.document,
        t.summary,
        similarity=sim
...
def get_document(doc_id: int):
    t = pxt.get_table('myapp/documents')
    result = t.where(t._rowid == doc_id).collect()
...
```bash
uvx pixeltable-new myapp --template content-pipeline   # document processing + RAG
uvx pixeltable-new myapp --template multimodal-rag      # cross-modal search (text, image, video)
uvx pixeltable-new myapp --template agent               # tool-calling agent with memory
uvx pixeltable-new myapp --template video-intel          # video frame analysis + search
uvx pixeltable-new myapp --template audio-intel          # audio transcription + search
uvx pixeltable-new myapp --template data-lab             # interactive data exploration
uvx pixeltable-new myapp --template full-stack-showcase  # FastAPI + React video surveillance app
```
...
| **[`templates/`](https://github.com/pixeltable/pixeltable-starter-kit/tree/main/templates)** | **Application Templates** | 7 vertical templates (content pipeline, RAG, agent, video, audio, data lab, full-stack showcase) scaffolded via `pixeltable-new --template`. |
