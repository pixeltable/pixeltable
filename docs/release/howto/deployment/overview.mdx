...
Generate a complete REST API from a TOML config. No FastAPI code, no frontend, no hand-written endpoints. Define your schema in Python, declare routes in the Pixeltable config file, and run `pxt serve`.
...
<AccordionGroup>
  ...
  <Accordion title="Architecture" icon="sitemap">
    ...
    - Declare routes in the Pixeltable config file using `[[pixeltable.service.routes]]`
...
type = "insert"
table = "pipeline.documents"
path = "/ingest-and-export"
inputs = ["title", "body"]
outputs = ["title", "body"]

[pixeltable.service.routes.export_sql]
