...
Hosted databases are backed up by Pixeltable Cloud; self-host still uses `pg_dump` of `~/.pixeltable/pgdata`.
...
- Back up your `app.py` and UDF code in version control
...
2. Recreate schema with `pxt schema update app.py TARGET`
