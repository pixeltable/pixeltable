...
Every Cloud database includes a managed bucket, and it is where the database keeps its media by default. No provider account, no credentials file. To send one computed column somewhere else, set `destination=` on it:
