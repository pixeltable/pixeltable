...
Or set `input_media_dest` / `output_media_dest` on the database's `[[pixeltable.database]]` entry to change the default for inserted or computed media. Details: [Cloud storage](/integrations/cloud-storage#pixeltable-cloud-home-bucket).
