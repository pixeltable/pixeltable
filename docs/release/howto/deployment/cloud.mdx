...
| A secret | `pxt secret set pxt://org:mydb KEY=...`, then `pxt db restart pxt://org:mydb` for the tables and `pxt service restart pxt://org:mydb/ingest` for each service, so their pods read it |
