...
Use the same `app.py` as the [Quickstart](/overview/quick-start). Create the hosted database, then the tables, then hosted HTTP.
...
`pxt db update` creates the hosted database or updates its image, secrets, and size. It does not insert your data and does not start app HTTP. `pxt schema update` creates the tables. It does not start HTTP. `pxt service update` starts hosted HTTP. Do not use `pxt service run` for Cloud; that command only starts HTTP in your local terminal.

After the database is up, insert from the Cloud dashboard and run `pxt schema diff` to see pending schema changes.
