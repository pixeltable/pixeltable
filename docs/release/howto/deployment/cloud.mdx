...
Every Cloud database includes a managed bucket, and it is where the database keeps its media by default: files you insert, files uploaded through an endpoint, and computed media all land there. No provider account, no credentials file. To send one computed column somewhere else, set `destination=` on it:
...
Or set `input_media_dest` / `output_media_dest` to change the default for inserted or computed media. Details: [Cloud storage](/integrations/cloud-storage#pixeltable-cloud-home-bucket).
