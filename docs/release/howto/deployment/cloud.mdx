...
Sign in from the browser, create a key, then export it. The CLI reads `PIXELTABLE_API_KEY`, which takes precedence over a `pxt login` session. To use the CLI without a key, run `pxt login` instead ([CLI](/platform/cli#signing-in)).
