...
Add a `[[pixeltable.service]]` section to the Pixeltable config file:
...
<Note>
To keep a service definition alongside a project rather than in the instance's config file, put it in its
own file with the same route syntax and name that file in `PIXELTABLE_CONFIG`:
`PIXELTABLE_CONFIG=service.toml pxt serve my-service`.
</Note>
...
This is identical to `add_insert_route`, except no row is inserted. For declarative
TOML/`pxt serve` configs, `type = "compute"` is currently treated as an insert alias;
use the Python `FastAPIRouter` API when you need true non-persisting compute.
...
Single-endpoint mode accepts the same fields as the equivalent `[[pixeltable.service.routes]]`
...
Settings for the FastAPI application and server. Each block defines one named service, which
`pxt serve <name>` and `pxt service create <name>` select by `name`.

| Field | Type | Default | Description |
|---|---|---|---|
| `name` | string | -- | Service name (required); may contain hyphens |
...
Each `[[pixeltable.service.routes]]` entry defines one HTTP endpoint. The `type` field determines
...
|---|---|---|---|---|
| `type` | `"insert"` / `"compute"` / `"update"` / `"delete"` / `"query"` | yes | -- | Route kind. In `pxt serve` TOML, `"compute"` is currently an alias for `"insert"`; use Python `add_compute_route` for non-persisting compute. |
...
```bash
pxt serve my-service                                # named service
pxt serve insert --table my_dir.my_table --path /gen --inputs prompt  # single endpoint
pxt serve my-service --dry-run                      # validate without starting
```
