...
The two combine: an application file may supply its own FastAPI app, and `pxt service` runs that one
instead of building its own. See [Your own application, run by `pxt service`](#your-own-application-run-by-pxt-service).
...
### Your own application, run by `pxt service`

An application file may hold your own `fastapi.FastAPI` object. `pxt service` then serves that application
as it is, so you keep the routes you wrote by hand and still get `diff`, `update`, `list` and `stop`
instead of running `uvicorn` yourself.

```python
# notes_app.py, under a project root
import fastapi
import pixeltable as pxt
...
```bash
pxt schema update notes_app.py my_dir     # create the tables the models declare
pxt service update notes_app.py my_dir    # serve the application, as service 'notes_app'
```

Three things to know:

- The file declares **one** service, named after its module (not the variable holding the app object).
- The models in the file are bound at the target before the application serves, and can be used in handler logic
  just like table names (e.g., `Notes.insert(...)`).
- All `FastAPIRouter` instances in the application file are part of the app service, not separate services of their own:
