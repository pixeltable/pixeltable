...
How the process starts is on [Self-hosting](/howto/deployment/overview). This page covers the route API once you have chosen which process serves it. After a service is running, FastAPI's OpenAPI UI is at `http://<host>:<port>/docs`, and the schema behind it is at `http://<host>:<port>/openapi.json`. Generate TypeScript types for a frontend from that schema:
