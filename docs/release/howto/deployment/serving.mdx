...
### Your own FastAPI app, run by `pxt service`

The application file may hold a `fastapi.FastAPI` object. `pxt service` then serves that app: one service, named after the module. Models bind at `TARGET` before it starts, so a handler can call `Notes.insert(...)`.
...
```bash
pxt schema update notes_app.py my_dir
pxt service update notes_app.py my_dir
```

Every `FastAPIRouter` in that file must be on `app` (`include_router`). A router left out is an error. Included routers are diffed by their declarations. Paths the app serves itself are diffed as path strings.
