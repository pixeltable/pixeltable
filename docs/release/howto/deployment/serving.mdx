...
Routes with `return_fileresponse=True` advertise a binary response for any media type. Check the response's `Content-Type` when handling the returned image, video, audio, or document; a fetch caller can consume its bytes with `response.blob()` or `response.arrayBuffer()`.

## Call from a TypeScript frontend
