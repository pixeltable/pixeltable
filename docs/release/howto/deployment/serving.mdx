...
A list of Python modules to import before resolving query function references.
Required when query routes reference `@pxt.query` or `retrieval_udf` functions
...
| `uploadfile_inputs` | list of strings | no | -- | Parameters to accept as file uploads |
...
Make sure the module is listed in `modules` so it gets imported before routes are resolved.

**Multi-row response** (default):

```bash
curl -X POST http://localhost:8000/search -d '{"query_text": "hello"}'
...
Plus the shared `--host`, `--port`, `--title`, `--prefix` service flags.
