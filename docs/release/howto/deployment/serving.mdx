...
For a hosted service, copy its full endpoint URL from the Cloud dashboard or find it with `pxt service list pxt://org:db --json` using an organization key. The gateway requires a credential to download the schema:
