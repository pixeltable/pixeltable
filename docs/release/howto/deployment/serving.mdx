...
Poll through your application server with the same Cloud key when hosted. The polling path is in the generated schema, so polling is a typed call on the same [TypeScript client](#call-from-a-typescript-app-server): `pixeltable.GET('/_pxt/jobs/{job_id}', { params: { path: { job_id: id } } })`. A failed job's `detail` has the same shape as an HTTP error's `detail`, so the same retry rule applies.
