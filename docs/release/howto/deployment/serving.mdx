...
`pxt service run app.py my_dir` serves them from the calling process instead, on one port, until
interrupted -- the mode for a container entrypoint or a development loop.
...
pxt service update app.py my_dir     # start the services in the background
pxt service run    app.py my_dir     # serve from this process instead, until interrupted
