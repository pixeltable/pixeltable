...
### Handling Nullable Columns in UDFs

When processing data that may contain `None` values, guard against nulls:
...
def safe_process(value: Optional[str]) -> str:
    """Handle nullable input gracefully."""
    if value is None:
        return ''
    return value.upper()  # Your actual processing
```

<Tip>
Pixeltable computed columns can produce `None` on errors (when `on_error='ignore'`). Always check `table.column.errortype` to distinguish between legitimate nulls and processing failures.
</Tip>
