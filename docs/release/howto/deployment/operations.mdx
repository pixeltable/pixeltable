...
<Tip>
**For standard insert, query, and delete endpoints,** consider [built-in HTTP serving](/howto/deployment/serving) with `pxt service` or `FastAPIRouter` before writing custom endpoint handlers. It handles request/response schemas, media serving, and background jobs automatically.
</Tip>
...
```
- Version control `app.py` like database migration scripts.
...
**Web Applications:**
- For standard endpoints, use [`pxt service`](/howto/deployment/serving) with an application file, or [`FastAPIRouter`](/howto/deployment/serving#quickstart-python)
- Run `pxt schema update app.py TARGET` once before starting workers to create tables
...
EXPOSE 8000
CMD ["sh", "-c", "uv run pxt schema update app.py my_app && uv run pxt service run app.py my_app"]
```
