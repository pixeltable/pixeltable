...
<Tabs>
  ...
  <Tab title="Google Gemini">
    ...
    t.add_computed_column(
        text=generate_content(contents=t.prompt, model='gemini-3.1-flash-lite').text
    )
    ```
...
<Tabs>
  ...
  <Tab title="Video/Audio">
    ...
    t.video.pan(x_sign=1)            # smooth pan right (convenience wrapper around scroll())
    t.video.mix_audio(t.music)       # blend a second audio track into the video
...
<Tabs>
  ...
  <Tab title="Export">
    ...
    # CSV / JSON
    io.export_csv(t, 'output.csv')
    io.export_json(t, 'output.json')
...
*Last updated: March 2026 | Pixeltable v0.5.x*
</Info>
