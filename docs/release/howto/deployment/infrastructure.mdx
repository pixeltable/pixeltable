...
Put models and routes in one application file. Apply schema, then serve.

**Application file (`app.py`):**
- `TableModel` classes for tables, views, computed columns, and indexes
- `FastAPIRouter` services for HTTP
- Apply once: `pxt schema update app.py TARGET`
- Serve: `pxt service update app.py TARGET` (or `pxt service run` in the foreground)

**Approach 2 (existing FastAPI app):** keep the same `app.py` models. Mount `FastAPIRouter` with `app.include_router()`. Do not run `python schema.py`.
...
embed = sentence_transformer.using(model_id=config.EMBEDDING_MODEL)
...
class Documents(TableModel, name='documents'):
    ...
    __indexes__ = [
        pxt.EmbeddingIndex(document, embedding=embed, name='docs_embed'),
    ]


api = FastAPIRouter(name='api', prefix='/api')
api.add_insert_route(
    Documents, path='/documents', inputs=[Documents.document, Documents.metadata]
)
```
...
To mount the same router on an existing FastAPI app:
...
├── app.py                 # TableModel classes + FastAPIRouter; pxt schema / pxt service
├── main.py                # Optional: FastAPI app that include_router()s app.py
...
def search_documents(query_text: str, limit: int = 5):
    ...
    sim = docs.document.similarity(string=query_text)
    return docs.order_by(sim, asc=False).limit(limit).select(docs.document, sim)


api = FastAPIRouter(name='api', prefix='/api')
api.add_insert_route(Documents, path='/documents', inputs=[Documents.document, Documents.metadata])
api.add_query_route(path='/search', query=search_documents)
```
  </Tab>

  <Tab title="main.py (Approach 2)">
```python
from fastapi import FastAPI
from app import api
...
<Note>
**Key Principles:**
- **One application file:** `app.py` declares tables and routes. `pxt schema update` creates tables; `pxt service update` / `run` serves them.
- **Approach 2:** `include_router()` the same `FastAPIRouter` on an existing FastAPI app. Apply schema with `pxt schema update` first — not `python schema.py`.
...
- **Multi-worker deployments:** Apply schema once (`pxt schema update app.py TARGET`) before starting workers. See [Starter Kit Dockerfile](https://github.com/pixeltable/pixeltable-starter-kit).
</Note>
...
*Declarative Serving (`pxt service`):*
- Same persistent storage as Full Backend.
- API routes declared with `FastAPIRouter` in the application file, no hand-written endpoint code.
...
- SQL databases: [`pxt.io.import_sql()`](/sdk/latest/io#func-import_sql) — see [SQL import/export cookbook](/howto/cookbooks/data/data-export-sql)
...
- SQL databases: [`pxt.io.export_sql(table, ...)`](/sdk/latest/io#func-export_sql) — see [SQL import/export cookbook](/howto/cookbooks/data/data-export-sql)
- Iceberg: [`pxt.io.export_iceberg(table, ...)`](/sdk/latest/io#func-export_iceberg) — see [analytics export cookbook](/howto/cookbooks/data/data-export-analytics)
