...
Separate schema definition from application code.

**Schema Definition (`schema.py`):**
- Defines directories, tables, views, computed columns, indexes, and `@pxt.query` functions
- Acts as Infrastructure-as-Code for Pixeltable entities
- Module-level definitions with `if_exists='ignore'` for idempotency
- Importing the module creates all tables (runs once per process)

**Application Code (`app.py`, `endpoints.py`, `functions.py`):**
- Imports `schema` to trigger table creation, then uses `pxt.get_table()` for handles
...
```python
# schema.py — creates tables, defines queries (flat, idempotent)
...
# app.py — import triggers table creation, then get handles
import schema  # noqa: F401 — side-effect import
import pixeltable as pxt
import config
...
├── schema.py              # Schema definition (tables, views, indexes, queries)
...
import pixeltable as pxt
import schema  # noqa: F401 — import triggers table creation
import config
...
def search(query: str, limit: int = 5) -> list[SearchResult]:
    results = schema.search_documents(query, limit).collect()
...
- **Built-in HTTP serving:** For standard endpoints, consider [`pxt serve`](/howto/deployment/serving) with a TOML config instead of manual `app.py` setup.
