...
Provider keys go under Secrets / `pxt secret`. A running database or service uses its startup
secret values. After `pxt secret set`, run `pxt db restart pxt://org:db` for the database, or
`pxt service restart pxt://org:db/<name>` for one hosted service, to load the new values.
