  ...
  <Tab title="5. Deploy">
    <Steps>
      ...
      <Step title="Deploy" icon="cloud">
        Serve your agent with `pxt service update`. Same file on Cloud: `pxt db update`, then `pxt schema update app.py pxt://org:db`, then `pxt service update app.py pxt://org:db`. [Deploy to Pixeltable Cloud](/howto/deployment/cloud).
