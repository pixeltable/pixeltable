  ...
  <Tab title="5. Deploy">
    <Steps>
      ...
      <Step title="Deploy" icon="cloud">
        Serve your agent locally with `pxt service update`. To put the same tables and HTTP on Pixeltable Cloud, run `pxt db update pxt://org:db`, then `pxt schema update app.py pxt://org:db`, then `pxt service update app.py pxt://org:db`. [Deploy to Pixeltable Cloud](/howto/deployment/cloud).
