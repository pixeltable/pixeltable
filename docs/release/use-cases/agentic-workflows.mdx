  ...
  <Tab title="5. Deploy">
    <Steps>
      ...
      <Step title="Deploy" icon="cloud">
        Serve your agent via `pxt service` locally. Apply the same file to Cloud with `pxt schema update app.py pxt://org:db` — [Deploy to Pixeltable Cloud](/howto/deployment/cloud). Managed HTTP on the hosted catalog is coming.
