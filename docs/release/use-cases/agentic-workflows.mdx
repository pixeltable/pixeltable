...
On Cloud, sign in with `pxt login` or [use an API key](/howto/deployment/cloud#get-an-api-key), then run `pxt db update`, `pxt schema update`, and `pxt service update` against `pxt://org:mydb`.

Search over chunks instead of tools: [RAG and live APIs](/use-cases/multimodal-backend).
