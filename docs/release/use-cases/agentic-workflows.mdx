...
On Cloud, sign in with `pxt login` or [use an API key](/cloud#get-an-api-key), then run `pxt db update`, `pxt schema update`, and `pxt service update` against `pxt://org:mydb`.
