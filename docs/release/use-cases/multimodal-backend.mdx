...
This page uses `pxt.create_table()` so you can run cells without a project. An app puts the same columns on a `TableModel` in `app.py` and creates the tables with `pxt schema update`.

<Note>
**Same foundation, different intent:** This workflow uses the same Pixeltable capabilities as [Media processing](/use-cases/media-processing) — tables, multimodal types, computed columns, iterators. The difference is the output: training datasets vs. live application intelligence.
</Note>
  ...
  <Tab title="5. Serve">
    <Steps>
      <Step title="Built-in HTTP Serving">
        Expose tables and queries as HTTP endpoints from the file that declares them

        [`pxt service`](/howto/deployment/serving), [`FastAPIRouter`](/howto/deployment/serving)
        ...
        class Images(TableModel, name='images'):
            image: pxt.Image
            caption = caption_image(image)  # a computed column

            __indexes__ = [pxt.EmbeddingIndex(caption, embedding=embed)]


        @pxt.query
        def search_images(text: str) -> pxt.Query:
            sim = Images.caption.similarity(string=text)
            return Images.order_by(sim, asc=False).limit(10)


        api = FastAPIRouter(name='image-service')
        api.add_insert_route(
            Images, path='/upload', uploadfile_inputs=[Images.image], outputs=[Images.caption]
        )
        api.add_query_route(path='/search', query=search_images)
        ```

        ```bash
        pxt schema update app.py my_dir      # create the tables the models declare
        pxt service update app.py my_dir     # serve the routes over them
        ```

        <CardGroup cols={2}>
          <Card title="HTTP Serving Guide" href="/howto/deployment/serving">
            Application file, CLI, Python API, background jobs
          </Card>
