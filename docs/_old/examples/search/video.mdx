...
<Steps>
  ...
  <Step title="Use Your Workflow" icon="play">
    ...
    audio_sim = transcription_chunks.text.similarity(string=query_text)
    audio_results = (
...
<CardGroup cols={1}>
  ...
  <Card title="Dual Search Capabilities" icon="magnifying-glass">
    ...
    audio_sim = transcription_chunks.text.similarity(string="Definition of happiness according to the guest")
    image_sim = frames_view.image_description.similarity(string="Lex Fridman interviewing a guest in a podcast setting")
    ```
  </Card>
</CardGroup>
