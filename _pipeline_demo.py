...
from _pipeline_udfs import (
    # Vision
    detect_objects, count_detections, check_person,
    classify_scene, assess_risk, estimate_pose, classify_action,
    # NLP
    detect_language, analyze_sentiment, extract_entities,
    summarize_text, classify_topic, score_quality,
    compute_readability, extract_phrases,
    # Agent
    parse_intent, assess_complexity, generate_plan,
    chain_of_thought, evaluate_confidence, make_decision,
    execute_action, flag_issues,
)
...
frames = pxt.create_table('vision/frames', {
    'frame_id': pxt.Int,
    'image_url': pxt.String,
...
person_frames.add_computed_column(
    action=classify_action(person_frames.pose_label, person_frames.scene_type)
)
print(f'  person_frames (view): {person_frames.count()} rows, +2 computed cols')

# View: high-risk alerts
alerts = pxt.create_view(
    'vision/alerts',
    frames.where(frames.risk_score > 0.6),
)
...
corpus = pxt.create_table('nlp/corpus', {
    'doc_id': pxt.Int,
    'title': pxt.String,
    'body': pxt.String,
    'source': pxt.String,
...
corpus.add_computed_column(
    quality_score=score_quality(corpus.body, corpus.sentiment, corpus.entities_json)
)
...
english = pxt.create_view(
    'nlp/english',
    corpus.where(corpus.language == 'en'),
)
english.add_computed_column(readability=compute_readability(english.body))
english.add_computed_column(
    key_phrases=extract_phrases(english.summary, english.entities_json)
)
print(f'  english (view): {english.count()} rows, +2 computed cols')

# View: Flagged low-quality
flagged = pxt.create_view(
    'nlp/flagged',
    corpus.where(corpus.quality_score < 0.5),
)
...
tasks = pxt.create_table('agent/tasks', {
    'task_id': pxt.String,
...
task_data = [
    ('T-001', 'Search for all documents related to machine learning in healthcare',
     'We have a corpus of 10K medical papers and clinical notes. Need to find relevant ML applications.',
     3),
    ('T-002', 'Create a summary report of quarterly earnings for all portfolio companies',
     'Portfolio includes TechCorp, BioGen, and DataVault. Q3 2025 financials are available.',
     2),
    ('T-003', 'Analyze sentiment trends across customer feedback channels',
     'Customer reviews from App Store, Google Play, and Zendesk tickets. Last 90 days.',
     1),
    ('T-004', 'Update the product recommendation model with new user data and then retrain',
     ...
     1),
    ('T-005', 'Find and fix all broken image URLs in the content database',
     'CMS database with 25K articles. Some images point to deprecated CDN.',
     2),
    ('T-006', 'Generate a comprehensive competitive analysis report',
     'Competitors: AlphaAI, BetaData, GammaML. Need pricing, features, market share analysis.',
     1),
    ('T-007', 'Delete all expired user sessions and clean up temporary files',
     'Redis cache has 100K session keys. File storage has /tmp artifacts older than 30 days.',
     3),
    ('T-008', 'Evaluate the performance of all deployed models across every production endpoint',
    ...
    ('T-009', 'x',
     'y',
     3),
    ('T-010', 'Analyze and then update the data pipeline to handle new sensor types and also '
...
} for i, (tid, instr, ctx, pri) in enumerate(task_data)], on_error='ignore')
print(f'  tasks: {tasks.count()} rows, 7 computed cols (deepest chain: 6 levels)')

# View: completed (decision != reject)
completed = pxt.create_view(
    'agent/completed',
    tasks.where(tasks.decision != 'reject'),
)
print(f'  completed (view): {completed.count()} rows')

# View: needs_review (confidence < 0.7) + extra computed col
needs_review = pxt.create_view(
    'agent/needs_review',
    tasks.where(tasks.confidence < 0.7),
)
needs_review.add_computed_column(
    reviewer_notes=flag_issues(needs_review.reasoning, needs_review.confidence)
)
