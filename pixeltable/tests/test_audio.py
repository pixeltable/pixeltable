...
class TestAudio:
    ...
    def test_basic(self, test_client: pxt.Client) -> None:
        ...
        status = audio_t.insert([[p] for p in audio_filepaths])
    ...
    def test_extract(self, test_client: pxt.Client) -> None:
        ...
        status = video_t.insert([[p] for p in video_filepaths], columns=['video'])
        assert status.num_rows == len(video_filepaths)
        assert status.num_excs == 0
        assert MediaStore.count(video_t.id) == len(video_filepaths) - 1
