...
class TestTable:
    ...
    def test_create_table_from_parquet(self, test_client : pt.Client, tmp_path : Path) -> None:
        parquet_dir = tmp_path / 'test_data'
        parquet_dir.mkdir()
        make_test_arrow_table(parquet_dir)

        ret = test_client.create_table_from_parquet('test_parquet', str(parquet_dir), explain_only=True)
        assert ret is None
        assert 'test_parquet' not in test_client.list_tables()

        tab = test_client.create_table_from_parquet('test_parquet', str(parquet_dir))
    ...
    def test_create_table_from_huggingface_dataset(self, test_client : pt.Client) -> None:
        test_cases = {
                    # use only a few rows for speed (relatively prime to any batch size)
                    'mnli':datasets.load_dataset("glue", "mnli", split="validation_matched[:13]"),
                    'c4':datasets.load_dataset("allenai/c4", data_files="en/c4-train.00001-of-01024.json.gz",
                                               split='train[:13]'), # too large to load all
        ...
        for dataset_name, hf_dataset in test_cases.items():
            tab = test_client.create_table_from_huggingface_dataset(dataset_name, hf_dataset, explain_only=True,
                                                                    split_column_name=split_column_name)
            assert tab is None

            tab = test_client.create_table_from_huggingface_dataset(dataset_name, hf_dataset,
                                                                    split_column_name=split_column_name)
            assert tab is not None
