...
class TestSnapshot:
    ...
    def test_views_of_snapshots(self, test_client) -> None:
        t = pxt.create_table('tbl', {'a': IntType()})
        rows = [{'a': 1}, {'a': 2}, {'a': 3}]
        status = t.insert(rows)
        assert status.num_rows == len(rows)
        assert status.num_excs == 0
        s1 = pxt.create_view('s1', t, is_snapshot=True)
        v1 = pxt.create_view('v1', s1, is_snapshot=False)
        s2 = pxt.create_view('s2', v1, is_snapshot=True)
        v2 = pxt.create_view('v2', s2, is_snapshot=False)
    ...
    def test_snapshot_of_view_chain(self, test_client) -> None:
        t = pxt.create_table('tbl', {'a': IntType()})
        rows = [{'a': 1}, {'a': 2}, {'a': 3}]
        status = t.insert(rows)
        assert status.num_rows == len(rows)
        assert status.num_excs == 0
        v1 = pxt.create_view('v1', t, is_snapshot=False)
        v2 = pxt.create_view('v2', v1, is_snapshot=False)
        s = pxt.create_view('s', v2, is_snapshot=True)
