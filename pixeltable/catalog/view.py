    ...
    def _create(
        ...
        if tbl_version is None:
            ...
            try:
                ...
                _. _, _, status = tbl_version.store_tbl.insert_rows(plan, v_min=tbl_version.version)
            except:
                ...
                raise
            Env.get().console_logger.info(f'Created view `{name}` with {status.num_rows} rows, {status.num_excs} exceptions.')
