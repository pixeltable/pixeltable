...
class TablePackager:
    ...
    def __export_table(self, tv: catalog.TableVersion) -> None:
        ...
        for col_md in tv.tbl_md.column_md.values():
            if col_md.stored and col_md.is_media_type:
...
class TableRestorer:
    ...
    def __import_table(self, bundle_path: Path, tv: catalog.TableVersion, tbl_md: TableVersionMd) -> None:
        ...
        media_col_names = {
            f'col_{col_md.id}' for col_md in tv.tbl_md.column_md.values() if col_md.stored and col_md.is_media_type
        }
        value_store_cols = [
    ...
    def __from_pa_pydict(self, tv: catalog.TableVersion, pydict: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for col_md in tv.tbl_md.column_md.values():
            if col_md.stored:
                stored_col_mds[Column.store_name_from_id(col_md.id)] = col_md
            if col_md.stores_cellmd:
                stored_col_mds[Column.cellmd_store_name_from_id(col_md.id)] = col_md
        ...
        for col_name, col_vals in pydict.items():
            ...
            is_media_col = col_md is not None and col_md.stored and col_md.is_media_type
            is_cellmd_col = (
                col_md is not None and col_md.stores_cellmd and col_name == Column.cellmd_store_name_from_id(col_md.id)
            )
            assert col_md is None or is_cellmd_col or col_name == Column.store_name_from_id(col_md.id)
