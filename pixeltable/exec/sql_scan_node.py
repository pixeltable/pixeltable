...
class SqlScanNode(ExecNode):
    ...
    def __init__(
            ...
            order_by_items: List[Tuple[exprs.Expr, bool]] = None,
            similarity_clause: Optional[exprs.ImageSimilarityPredicate] = None,
            limit: int = 0, set_pk: bool = False, exact_version_only: List[catalog.TableVersion] = None
    ):
        """
        Args:
    ...
    def create_from_clause(
            cls, tbl: catalog.TableVersionPath, stmt: sql.Select, refd_tbl_ids: Set[UUID] = None,
            exact_version_only: Set[UUID] = None
    ) -> sql.Select:
