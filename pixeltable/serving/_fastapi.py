...
class FastAPIRouter(fastapi.APIRouter):
    ...
    `FastAPIRouter` is for apps that already have a FastAPI server. To start a
    Pixeltable HTTP service without writing a FastAPI app, use `pxt serve` from
    the CLI. Learn more here: [HTTP Serving](https://docs.pixeltable.com/howto/deployment/serving).
    """
