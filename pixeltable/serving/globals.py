...
class SqlExport(pydantic.BaseModel):
    ...
    def display_dict(self) -> dict[str, Any]:
        """This target as a dict, with db_connect reduced to a digest of itself.

        db_connect holds the credentials of the database it names, so a caller that stores or reports a
        target uses this instead of model_dump(). A changed connection string changes the digest.
        """
        d = self.model_dump()
        d['db_connect_digest'] = hashlib.sha256(d.pop('db_connect').encode()).hexdigest()[:16]
        return d
