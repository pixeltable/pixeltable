...
def pan(video: Any, *, direction: Literal['left', 'right', 'up', 'down'] = 'right', crop_pct: float = 0.2) -> Any:
    ...
    if crop_pct <= 0.0 or crop_pct >= 1.0:
        raise pxt.RequestError(
            pxt.ErrorCode.INVALID_ARGUMENT, f'crop_pct must be between 0.0 and 1.0 (exclusive), got {crop_pct}'
        )
    ...
    if direction in ('left', 'right'):
        ...
        raise pxt.RequestError(
            pxt.ErrorCode.INVALID_ARGUMENT, f"direction must be one of 'left', 'right', 'up', 'down', got {direction!r}"
        )
