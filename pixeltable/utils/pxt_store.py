...
# pxt_store clients are thread-local via Runtime._clients, matching r2/s3/b2/tigris.
...
def _build_pxt_store_entry(org: str, db: str) -> _PxtStoreCacheEntry:
    ...
    # Supply our credentials as the sole provider in a fresh botocore session.
    # register_component replaces the lazily-registered default chain before it is built,
    # so the session never falls through to env vars or config files.
...
def _get_or_create_pxt_store_entry(org: str, db: str) -> _PxtStoreCacheEntry:
    ...
    cd = get_runtime().get_client('pxt_store')
