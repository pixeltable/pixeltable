...
def to_pixeltable_type(arrow_type: pa.DataType) -> Optional[ts.ColumnType]:
    ...
    if arrow_type in _pa_to_pt:
        ...
        return ts.ArrayType(shape=arrow_type.shape, dtype=to_pixeltable_type(arrow_type.value_type))
    else:
        return None
...
def to_arrow_schema(pixeltable_schema: Dict[str, Any]) -> pa.Schema:
    return pa.schema({name: to_arrow_type(dtype) for name, dtype in pixeltable_schema.items()})
