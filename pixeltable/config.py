...
class Config:
    ...
    def __init__(self, config_overrides: dict[str, Any]) -> None:
        assert (
            self.__instance is None
        ), "Config is a singleton; use Config.get() to access the instance"
        ...
        self.__home = Path(
            self.lookup_env("pixeltable", "home", str(Path.home() / ".pixeltable"))
        )
        ...
        self.__config_file = Path(
            self.lookup_env("pixeltable", "config", str(self.__home / "config.toml"))
        )
        ...
        if os.path.isfile(self.__config_file):
            with open(self.__config_file, "r", encoding="utf-8") as stream:
                try:
                    ...
                    raise excs.Error(
                        f"Could not read config file: {self.__config_file}"
                    ) from exc
            ...
            with open(self.__config_file, "w", encoding="utf-8") as stream:
                try:
                    ...
                    raise excs.Error(
                        f"Could not write config file: {self.__config_file}"
                    ) from exc
    ...
    def get_value(
        self, key: str, expected_type: type[T], section: str = "pixeltable"
    ) -> T | None:
...
KNOWN_CONFIG_OPTIONS = {
    ...
    "deepseek": {
        ...
        "rate_limit": "Rate limit for Deepseek API requests",
    },
    "fal": {
        "api_key": "fal.ai API key",
        "rate_limit": "Rate limit for fal.ai API requests",
    },
    "fireworks": {
        "api_key": "Fireworks API key",
        "rate_limit": "Rate limit for Fireworks API requests",
    },
    "gemini": {
    ...
    "reve": {
        ...
        "rate_limit": "Rate limit for Reve API requests (requests per minute)",
    },
    "groq": {
        "api_key": "Groq API key",
        "rate_limit": "Rate limit for Groq API requests",
    },
    "label_studio": {
    ...
    "mistral": {
        ...
        "rate_limit": "Rate limit for Mistral API requests",
    },
    "openai": {
    ...
    "twelvelabs": {
        ...
        "rate_limit": "Rate limit for TwelveLabs API requests",
    },
    "veo": {"rate_limits": "Per-model rate limits for Veo API requests"},
    "voyage": {
        "api_key": "Voyage AI API key",
        "rate_limit": "Rate limit for Voyage AI API requests",
    },
