...
class AggregateFunction(Function):
    ...
    """
    RESERVED_PARAMS = {'order_by', 'group_by'}
    ...
    def __init__(
        ...
        init_params = [
            ...
            for p in list(inspect.signature(self.agg_cls.__init__).parameters.values())[1:]  # leave out self
        ]
    ...
    def validate_call(self, bound_args: Dict[str, Any]) -> None:
        ...
        for param_name in self.init_param_names:
            if param_name in bound_args and isinstance(bound_args[param_name], exprs.Expr):
                raise excs.Error(f'init() parameter {param_name} needs to be a constant, not a Pixeltable expression')
