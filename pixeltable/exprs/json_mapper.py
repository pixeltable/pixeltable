...
class JsonMapperDispatch(Expr):
    ...
    def eval(self, data_row: DataRow, row_builder: RowBuilder) -> None:
        ...
        raise AssertionError('INTERNAL ERROR: JsonMapperDispatch.eval() should never be called.')

    def _as_dict(self) -> dict:
        """
        JsonMapperDispatch instances are only created by the JsonMapper c'tor and never need to be serialized.
        """
        raise AssertionError('INTERNAL ERROR: JsonMapperDispatch._as_dict() should never be called.')

    @classmethod
    def _from_dict(cls, d: dict, components: list[Expr]) -> JsonMapperDispatch:
        raise AssertionError('INTERNAL ERROR: JsonMapperDispatch._from_dict() should never be called.')
