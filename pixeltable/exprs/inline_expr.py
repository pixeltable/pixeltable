...
class InlineList(Expr):
    ...
    def __init__(self, elements: Iterable):
        ...
        type_schema = ts.JsonType.TypeSchema(content=[expr.col_type for expr in exprs])
...
class InlineDict(Expr):
    ...
    def __init__(self, d: dict[str, Any]):
        ...
        type_schema = ts.JsonType.TypeSchema(content={key: expr.col_type for key, expr in zip(self.keys, exprs)})
        super().__init__(ts.JsonType(type_schema))
        self.components.extend(exprs)
        self.id = self._create_id()
