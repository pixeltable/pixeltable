...
def _api_key_source() -> str:
    """Where Config found the API key, in the order it looks: pxt.init(), the environment, a config file."""
    config = Config.get()
    if config.is_overridden('api_key'):
        return 'pxt.init()'
    if isinstance(config.get_value_source('api_key'), Path):
        return 'api_key in the Pixeltable config file'
    return 'the PIXELTABLE_API_KEY environment variable'
