...
def run_films_demo() -> None:
    ...
    with logfire.span("films_demo", _span_name="📽️ Films Database Demo"):
        ...
        with logfire.span("create_films_table"):
            films = pxt.create_table('telemetry_demo.films', {
                'title': pxt.String,
                'year': pxt.Int,
                'revenue_millions': pxt.Float,
                'genre': pxt.String,
            })
        ...
        with logfire.span("add_computed_column"):
            films.add_computed_column(
                revenue_billions=films.revenue_millions / 1000.0
            )
        ...
        with logfire.span("query_by_genre"):
            action_films = films.where(films.genre == 'Action').select(
                films.title, films.year, films.revenue_millions
            ).order_by(films.revenue_millions, asc=False).collect()
        ...
        with logfire.span("update_films"):
            films.update(
                {'revenue_millions': films.revenue_millions * 1.05},
                where=films.year == 2024
            )
...
def run_video_detection_demo() -> None:
    ...
    with logfire.span("video_detection_demo", _span_name="🎬 Video Detection Demo"):
        ...
        with logfire.span("create_videos_table"):
            videos = pxt.create_table('telemetry_demo.videos', {
                'video': pxt.Video,
...
def run_similarity_search_demo() -> None:
    ...
    with logfire.span("similarity_search_demo", _span_name="🔍 Similarity Search Demo"):
        ...
        with logfire.span("create_images_table"):
            images = pxt.create_table('telemetry_demo.images', {
                'image': pxt.Image,
                'category': pxt.String,
            })
        ...
        with logfire.span("insert_images"):
            ...
            images.insert([
                {'image': url, 'category': cat}
                for url, cat in image_data
            ])
        ...
        with logfire.span("text_similarity_search"):
            ...
            results = images.select(
                ...
                images.category,
                sim
            ).order_by(sim, asc=False).limit(3).collect()
...
def run_analytics_demo() -> None:
    ...
    with logfire.span("analytics_demo", _span_name="📊 Analytics Demo"):
        ...
        with logfire.span("add_category_column"):
            films.add_computed_column(
                is_blockbuster=(films.revenue_millions > 1000),
                if_exists='replace'
            )
        ...
        with logfire.span("query_by_decade"):
            films_2020s = films.where(
                (films.year >= 2020) & (films.year < 2030)
            ).select(films.title, films.year).collect()
            logfire.info(f"Films from 2020s: {len(films_2020s)}")
        
        # Complex query: Genre + revenue filter
        with logfire.span("complex_filter_query"):
            action_hits = films.where(
                (films.genre == 'Action') & (films.revenue_millions > 1300)
            ).select(films.title, films.revenue_millions).collect()
