"""
Auto-generates type stubs for pixeltable.catalog.model.definition based on FORWARDED_TABLE_METHODS.

This script first uses `stubgen` to generate stubs for both `definition.py` and `table.py`, then merges the relevant
method definitions from the `Table` class in `table.pyi` into the `TableModelMeta` definition in `definition.pyi`.
...
from pixeltable.catalog.model.definition import FORWARDED_TABLE_METHODS


def generate_stubs() -> None:
    subprocess.run(('stubgen', '-m', 'pixeltable.catalog.model.definition', '-o', 'target'), check=True)
    subprocess.run(('stubgen', '-m', 'pixeltable.catalog.table', '-o', 'target'), check=True)


def merge_stubs() -> None:
    with open('target/pixeltable/catalog/model/definition.pyi', 'r', encoding='utf-8') as f:
    ...
    assert class_defn_idx is not None, '`TableModelMeta` class definition not found in definition.pyi'
    generated_stub = [
    ...
    with open('pixeltable/catalog/model/definition.pyi', 'w', encoding='utf-8') as f:
        f.writelines(generated_stub)


def main() -> None:
    print('Generating type stubs for pixeltable.catalog.model.definition ...')
    generate_stubs()
    print('Merging forwarded method definitions from table.pyi into definition.pyi ...')
    merge_stubs()
    print('Done.')
