...
# Building a Visual Search Pipeline

Pixeltable image search works in two phases:
1. Define your pipeline structure (once)
...
<Steps>
  ...
  <Step title="Define Your Pipeline" icon="wand-magic-sparkles">
    Create `schema.py`:
    ```python
  ...
  <Step title="Use Your Pipeline" icon="play">
    Create `app.py`:
    ```python
    import pixeltable as pxt
