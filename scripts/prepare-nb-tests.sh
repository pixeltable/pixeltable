...
rm -f "$TARGET_DIR/working-with-gemini.ipynb"  # Temporary
rm -f "$TARGET_DIR/rag-operations.ipynb"  # Failing in CI for unknown reasons
rm -f "$TARGET_DIR/video-generate-ai.ipynb"  # Expensive
rm -f "$TARGET_DIR/img-image-to-image.ipynb"  # Expensive (downloads ~5GB model)
