...
if [ -n "$PXT_STRESS_TESTS_READ_ONLY" ]; then
    ...
    python -u -m tool.random_ops "$WORKERS" "$DURATION" --read-only-workers $(( WORKERS - 1 ))
else
    # In read/write mode, we disable certain operations and data types that have known concurrency bugs.
    python -u -m tool.random_ops "$WORKERS" "$DURATION" -Drandom_img_freq=0 -Drandom_json_freq=0 -Drandom_array_freq=0
fi
