...
pixeltable/              # Library source
  ...
  iterators/             #   View iterators (document_splitter, frame_iterator)
...
**Flag any PR that introduces `is None` or `not` in a Pixeltable expression context.** This is the single most dangerous class of bug.
...
- When custom None-handling is needed, annotate with `Optional[X]` and guard explicitly:
...
**Adding an iterator**: Extend `ComponentIterator` in `pixeltable/iterators/`. Implement `input_schema()`, `output_schema()`, `__next__()`, `close()`.
...
### Don't Expose Implementation Complexity to Users

If a user-facing API requires `.localpath` or other internal accessors, the API itself should be fixed rather than documented around. Flag any PR that pushes internal complexity to the user (e.g., requiring `tbl.video_col.localpath` instead of `tbl.video_col`).
...
- [ ] `make format` has been run (no formatting diff).
- [ ] `make check` passes (mypy + ruff + notebook checks).
- [ ] New/changed functions have complete type annotations.
...
- [ ] If docs changed: `make docs` passes with no broken links.
