...
Two kinds share the command because they are the same thing to a user asking for a key:
...
def _fetch() -> tuple[list[dict], list[dict]]:
    ...
    return resp.get('api_keys', []), resp.get('runtime_keys', [])


def _list(args: argparse.Namespace) -> None:
    api_keys, runtime_keys = _fetch()
    if args.json_output:
        print(json.dumps({'api_keys': api_keys, 'runtime_keys': runtime_keys}, indent=2))
        return
    if not api_keys and not runtime_keys:
        print('No keys.')
        return
    for key in api_keys:
        _render(key.get('name', ''), 'user', None)
    for key in runtime_keys:
        _render(key.get('name', ''), 'runtime', key.get('grants') or [])


def _create(args: argparse.Namespace) -> None:
    if args.grant is None:
        resp = post_request('/api/keys', {'name': args.name})
        body = resp if isinstance(resp, dict) else {}
        if args.json_output:
            print(json.dumps(body, indent=2))
            return
        _render(body.get('name', args.name), 'user', None)
        _secret(body.get('api_key'))
        return

    resp = post_request('/api/runtime-keys', {'name': args.name, 'grants': _grants(args.grant, '--grant')})
    key = resp.get('runtime_key', {}) if isinstance(resp, dict) else {}
    if args.json_output:
        print(json.dumps(key, indent=2))
        return
    _render(key.get('name', args.name), 'runtime', key.get('grants') or [])
...
def _update(args: argparse.Namespace) -> None:
    ...
    if not grant and not revoke:
        ...
        sys.exit(2)
    resp = post_request('/api/runtime-keys/update', {'name': args.name, 'allow': grant, 'revoke': revoke})
    key = resp.get('runtime_key', {}) if isinstance(resp, dict) else {}
    if args.json_output:
        print(json.dumps(key, indent=2))
        return
...
def _delete(args: argparse.Namespace) -> None:
    """Names are unique across both kinds, so the listing settles which endpoint to call."""
    api_keys, runtime_keys = _fetch()
    if any(k.get('name') == args.name for k in api_keys):
        post_request('/api/keys/delete', {'name': args.name})
    elif any(k.get('name') == args.name for k in runtime_keys):
        post_request('/api/runtime-keys/delete', {'name': args.name})
    else:
        print(f'pxt key delete: error: no key named {args.name!r}', file=sys.stderr)
        sys.exit(1)
