...
# ── Directories ──────────────────────────────────────────────────────────────
dirs = [
    'demo', 'demo.content', 'demo.content.articles', 'demo.content.docs',
    'demo.analytics', 'demo.analytics.web', 'demo.analytics.product',
    'demo.ml', 'demo.ml.training', 'demo.ml.evaluation',
    'demo.ecommerce', 'demo.ecommerce.catalog', 'demo.ecommerce.orders',
    'demo.ecommerce.reviews',
    'demo.social', 'demo.social.posts', 'demo.social.moderation',
]
for d in dirs:
...
def extract_tags(text: str) -> str:
    keywords = ['AI', 'ML', 'data', 'model', 'training', 'pipeline', 'multimodal',
                'vector', 'embedding', 'RAG', 'LLM', 'image', 'video', 'audio']
...
# ── 1. Articles ──────────────────────────────────────────────────────────────
articles = pxt.create_table('demo.content.articles.posts', {
    'title': pxt.String, 'body': pxt.String,
    'author': pxt.String, 'category': pxt.String,
    'published': pxt.Timestamp,
})

article_data = [
    ('Getting Started with Pixeltable', 'Pixeltable unifies data storage and AI orchestration in one declarative framework. With computed columns and automatic lineage tracking you can build multimodal AI pipelines without glue code. This tutorial walks you through creating your first table, adding computed columns, and querying results. The framework handles all the complexity of data versioning and dependency management behind the scenes.', 'Alice', 'tutorial'),
    ('Building RAG Pipelines', 'Retrieval-Augmented Generation combines document retrieval with language model generation. Pixeltable makes this easy with embedding indices and computed columns that chain together retrieval and generation steps. You can build a complete RAG pipeline in just a few lines of code, with automatic caching and incremental updates.', 'Bob', 'rag'),
    ('Multimodal Data Management', 'Managing images, videos, audio, and text together is challenging. Pixeltable provides a unified interface for all media types with automatic format handling and lazy loading. Store everything in one table and let computed columns handle the transformations.', 'Charlie', 'data'),
    ('Vector Search at Scale', 'Embedding indices in Pixeltable enable fast similarity search across millions of records. Combined with computed columns that generate embeddings automatically, you get a production-ready vector search system with zero infrastructure overhead.', 'Alice', 'ml'),
    ('Computer Vision Pipelines', 'Build end-to-end computer vision pipelines with Pixeltable. From image ingestion to model inference, everything is declarative. Add object detection, classification, or segmentation as computed columns and results are cached automatically.', 'Diana', 'vision'),
    ('LLM Integration Patterns', 'Integrating large language models into your data pipeline is straightforward with Pixeltable. Use computed columns to call OpenAI, Anthropic, or local models. Results are cached, versioned, and queryable just like any other column.', 'Eve', 'llm'),
    ('Data Versioning Best Practices', 'Every change in Pixeltable is versioned automatically. This guide covers how to leverage versioning for reproducible experiments, A/B testing, and rollback capabilities. Learn to use snapshots and views for different use cases.', 'Bob', 'data'),
    ('Audio Processing with Pixeltable', 'Process audio files at scale using Pixeltable computed columns. From transcription to speaker diarization, build complete audio analysis pipelines declaratively. Support for WAV, MP3, FLAC and more.', 'Frank', 'audio'),
    ('Fine-tuning Models with Pixeltable', 'Use Pixeltable as your data backend for model fine-tuning. Manage training data, track experiments, and version your datasets. Integration with PyTorch DataLoaders makes it easy to feed data into your training loop.', 'Alice', 'ml'),
    ('Document Intelligence', 'Extract structured data from PDFs, contracts, and reports using Pixeltable document processing pipelines. Combine OCR, layout analysis, and LLM extraction in a single declarative pipeline.', 'Grace', 'document'),
    ('Real-time Data Pipelines', 'Pixeltable supports incremental processing for real-time applications. As new data arrives, computed columns update automatically. Learn how to build streaming-like pipelines with batch efficiency.', 'Charlie', 'data'),
    ('Sentiment Analysis at Scale', 'Build a sentiment analysis pipeline that processes thousands of reviews per minute. Pixeltable handles batching, caching, and error recovery automatically so you can focus on the model logic.', 'Diana', 'nlp'),
    ('Image Generation Workflows', 'Manage image generation workflows with Pixeltable. Store prompts, parameters, and generated images together. Use computed columns to run DALL-E, Stable Diffusion, or Midjourney and track everything automatically.', 'Eve', 'vision'),
    ('Multi-Agent Systems', 'Orchestrate multi-agent AI systems with Pixeltable as the shared memory layer. Agents can read and write to tables, with computed columns triggering downstream processing automatically.', 'Frank', 'agents'),
    ('Production Deployment Guide', 'Deploy Pixeltable-powered applications to production. This guide covers FastAPI integration, scaling strategies, monitoring, and best practices for running Pixeltable in a production environment.', 'Grace', 'deployment'),
]

articles.insert([{
    'title': t, 'body': b, 'author': a, 'category': c,
...
pages = pxt.create_table('demo.content.docs.pages', {
    'title': pxt.String, 'content': pxt.String,
    'section': pxt.String, 'order_idx': pxt.Int,
})

sections = ['Getting Started', 'Core Concepts', 'API Reference', 'Tutorials',
            'Deployment', 'Advanced', 'FAQ', 'Changelog']
for si, section in enumerate(sections):
    pages.insert([{
        'title': f'{section} - Part {j+1}',
        'content': f'Documentation for {section.lower()} part {j+1}. ' * 20,
        'section': section, 'order_idx': si * 10 + j,
    } for j in range(4)])
...
# ── 3. Web Pageviews ────────────────────────────────────────────────────────
pageviews = pxt.create_table('demo.analytics.web.pageviews', {
    'url': pxt.String, 'referrer': pxt.String,
    'user_agent': pxt.String, 'duration_sec': pxt.Float,
...
# ── 4. Daily Metrics ────────────────────────────────────────────────────────
metrics = pxt.create_table('demo.analytics.web.daily_metrics', {
    'date': pxt.Date, 'page': pxt.String,
    'visitors': pxt.Int, 'pageviews': pxt.Int,
    'avg_duration': pxt.Float, 'bounce_rate': pxt.Float,
})
...
# ── 5. Product Events ───────────────────────────────────────────────────────
events = pxt.create_table('demo.analytics.product.events', {
    'event_type': pxt.String, 'user_id': pxt.String,
    'properties': pxt.Json, 'timestamp': pxt.Timestamp,
})

event_types = ['page_view', 'button_click', 'form_submit', 'signup',
               'login', 'purchase', 'error', 'search']

events.insert([{
    'event_type': random.choice(event_types),
    'user_id': f'user_{random.randint(1, 50):03d}',
    'properties': {'screen': random.choice(['home', 'settings', 'profile']),
...
# ── 6. ML Training Runs ─────────────────────────────────────────────────────
runs = pxt.create_table('demo.ml.training.runs', {
    'model_name': pxt.String, 'run_id': pxt.String,
    'epoch': pxt.Int, 'loss': pxt.Float, 'accuracy': pxt.Float,
    'learning_rate': pxt.Float, 'batch_size': pxt.Int,
...
results = pxt.create_table('demo.ml.evaluation.results', {
    'model_name': pxt.String, 'dataset': pxt.String,
    'metric': pxt.String, 'value': pxt.Float,
...
products = pxt.create_table('demo.ecommerce.catalog.products', {
    'name': pxt.String, 'description': pxt.String,
    'category': pxt.String, 'price': pxt.Float,
    'stock': pxt.Int, 'rating': pxt.Float,
})
...
# ── 9. Orders ────────────────────────────────────────────────────────────────
orders = pxt.create_table('demo.ecommerce.orders.transactions', {
    'order_id': pxt.String, 'customer_id': pxt.String,
    'product_name': pxt.String, 'quantity': pxt.Int,
    'unit_price': pxt.Float, 'status': pxt.String,
...
# ── 10. Reviews ──────────────────────────────────────────────────────────────
reviews = pxt.create_table('demo.ecommerce.reviews.feedback', {
    'product_name': pxt.String, 'reviewer': pxt.String,
    'rating': pxt.Int, 'text': pxt.String,
    'helpful_votes': pxt.Int, 'posted_at': pxt.Timestamp,
})
...
# ── 11. Social Posts ─────────────────────────────────────────────────────────
community = pxt.create_table('demo.social.posts.community', {
    'author': pxt.String, 'content': pxt.String,
    'likes': pxt.Int, 'shares': pxt.Int, 'comments': pxt.Int,
    'platform': pxt.String, 'posted_at': pxt.Timestamp,
})
...
# ── 12. Moderation Flags ────────────────────────────────────────────────────
flags = pxt.create_table('demo.social.moderation.flags', {
    'post_id': pxt.Int, 'reason': pxt.String,
    'severity': pxt.String, 'reviewed': pxt.Bool,
