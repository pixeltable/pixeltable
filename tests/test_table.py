...
class TestTable:
    ...
    def test_image_table(self, reset_db: None) -> None:
        ...
        tbl = pxt.create_table('test', schema)
        assert ObjectOps.count_default_input_dest(tbl._id) == 0
        ...
        assert ObjectOps.count_default_input_dest(tbl._id) == n_sample_rows
        ...
        if Env.get().default_input_media_dest == Env.get().default_output_media_dest:
            assert ObjectOps.count_default_input_dest(tbl._id) == 2 * n_sample_rows
    ...
    def test_create_video_table(self, reset_db: None) -> None:
        ...
        assert ObjectOps.count_default_output_dest(view._id) == view.count() * 2
    ...
    def _test_computed_img_cols(self, t: pxt.Table, stores_img_col: bool) -> None:
        ...
        _ = t.count()
        _ = t.show()
        assert ObjectOps.count_default_output_dest(t._id) == t.count() * stores_img_col
        ...
        # make sure we can still insert data and that computed cols are still set correctly
        t2.insert(rows)
        assert ObjectOps.count_default_output_dest(t2._id) == t2.count() * stores_img_col
        _ = t2.collect()
