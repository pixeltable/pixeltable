"""Tests for `pxt serve` and `pxt deploy` (pxt_cli/client/commands/).
...
from pxt_cli.client.main import main as cli_main
...
def _run_cli(
    ...
    *,
    ...
    with patch('sys.argv', argv), patch('pxt_cli.client.commands.serve.config.Config.init', _init_with_reinit):
        try:
            cli_main()
...
class TestCLI:
    ...
    def test_serve_routes(self, tmp_path: pathlib.Path) -> None:
        ...
        with (
            patch('pxt_cli.client.commands.serve.lookup_service_config') as mock_load,
            patch('pxt_cli.client.commands.serve.create_service_from_config') as mock_create,
            ...
            with (
                ...
                patch('pxt_cli.client.commands.serve.config.Config.init', _init_with_reinit),
            ):
                cli_main()
    ...
    def test_serve_output(self, capsys: pytest.CaptureFixture) -> None:
        ...
        with patch('pxt_cli.client.commands.serve.create_service_from_config') as mock_create, patch('uvicorn.run'):
            mock_create.return_value = 'fake_app'
        ...
        with patch(
            'pxt_cli.client.commands.serve.create_service_from_config',
        ...
        with (
            patch('pxt_cli.client.commands.serve.create_service_from_config', return_value='fake_app'),
    ...
    def test_deploy(self, capsys: pytest.CaptureFixture) -> None:
        ...
        with patch('pxt_cli.client.commands.deploy.deploy.build_deploy_bundle') as mock_build:
