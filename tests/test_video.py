...
class TestVideo:
    ...
    def test_pan_errors(self, uses_db: None) -> None:
        ...
        with pxt_raises(pxt.ErrorCode.INVALID_ARGUMENT, match=r'crop_pct must be between'):
            t.select(pan(t.video, crop_pct=0.0)).collect()
        with pxt_raises(pxt.ErrorCode.INVALID_ARGUMENT, match=r'crop_pct must be between'):
            t.select(pan(t.video, crop_pct=1.0)).collect()
        with pxt_raises(pxt.ErrorCode.INVALID_ARGUMENT, match=r'direction must be one of'):
