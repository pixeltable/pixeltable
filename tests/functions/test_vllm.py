...
class TestVllm:
    def test_chat_completions(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=vllm.chat_completions(
                t.messages, model='Qwen/Qwen2.5-0.5B-Instruct', engine_kwargs={'max_model_len': 512}
            )
        )
        t.add_computed_column(
            output2=vllm.chat_completions(
                ...
                sampling_params={'max_tokens': 100, 'temperature': 0.7, 'top_p': 0.9, 'top_k': 20},
            )
        )
        ...
        assert len(result2['outputs'][0]['text']) > 0
    ...
    def test_generate(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=vllm.generate(t.prompt, model='Qwen/Qwen2.5-0.5B-Instruct', engine_kwargs={'max_model_len': 512})
        )
