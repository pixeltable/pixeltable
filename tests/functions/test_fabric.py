...
def _skip_if_not_fabric() -> None:
    ...
    try:
        from synapse.ml.fabric.service_discovery import get_fabric_env_config  # type: ignore[import-not-found]
...
class TestFabric:
    ...
    def test_chat_completions_standard_model(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=fabric.chat_completions(
                messages, model='gpt-4.1', model_kwargs={'max_tokens': 100, 'temperature': 0.7}
            )
        )
    ...
    def test_chat_completions_reasoning_model(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=fabric.chat_completions(messages, model='gpt-5', model_kwargs={'max_completion_tokens': 100})
        )
    ...
    def test_chat_completions_with_max_tokens_conversion(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=fabric.chat_completions(
                messages,
                model='gpt-5',
                model_kwargs={'max_tokens': 100},  # Should be converted to max_completion_tokens
            )
        )
    ...
    def test_chat_completions_custom_api_version(self, uses_db: None) -> None:
        ...
        t.add_computed_column(
            output=fabric.chat_completions(messages, model='gpt-4.1', api_version='2024-02-15-preview')
        )
