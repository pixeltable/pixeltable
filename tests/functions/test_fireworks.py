...
class TestFireworks:
    def test_fireworks(self, uses_db: None) -> None:
        ...
        validate_update_status(t.insert(input='What is the capital of Germany?'), 1)
        results = t.collect()
        assert 'Berlin' in results['output'][0]['choices'][0]['message']['content']
        assert 'Berlin' in results['output_2'][0]['choices'][0]['message']['content']
