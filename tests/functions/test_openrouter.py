...
class TestOpenRouter:
    ...
    def test_tool_invocations(self, reset_db: None) -> None:
        ...
        from tests.functions.tool_utils import calculator_tool_spec, calculator_impl
        ...
        @pxt.udf(**calculator_tool_spec)
        def calculator(**kwargs) -> dict:
            return calculator_impl(**kwargs)

        tools = pxt.tools(calculator)
        t = make_table(tools, 'auto')
        validate_update_status(
            t.insert(input='How much is 123 * 456?'),
            1
        )
        ...
        assert 'calculator' in tool_result
        calc_result = tool_result['calculator']
        assert calc_result['result'] == 56088
