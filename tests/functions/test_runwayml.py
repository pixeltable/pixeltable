...
class TestRunwayML:
    ...
    def test_image_to_video_smoke(self, reset_db: None) -> None:
        ...
        t = pxt.create_table(
            "test_tbl", {"image_url": pxt.String, "prompt": pxt.String}
        )
    ...
    def test_text_to_image_turbo(self, reset_db: None) -> None:
        ...
        validate_update_status(
            t.insert(prompt="A colorful abstract painting", ref_images=[ref_url]), 1
        )
        results = t.collect()
        assert results["output"][0] is not None
        assert "output" in results["output"][0]
...
class TestRunwayMLErrors:
    ...
    def test_missing_api_key(
        self, reset_db: None, monkeypatch: pytest.MonkeyPatch
    ) -> None:
